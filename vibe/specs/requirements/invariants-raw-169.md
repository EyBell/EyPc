...
前八块合计使入口从 15,046 降到 **14,737**（-309 行，2.1%），并建立 `preload/codex/` 八个模块。按已识别的候选簇估算，该路线尚可再降约 800–1,200 行；要越过这条线必须动 `codexActivityInventory` 等跨域缓存，那是路线 2 或 3 的范围，需先明确验收口径。
...
已识别的下批候选簇（均零共享绑定）：environment TOML 解析（2 函数 / 89 行）、`codexLaunchPlan` 闭包（5 函数 / 71 行）、`codexDesktopProjectedRequest` 闭包（7 函数 / 72 行）、`sanitizeCodexQuota` 闭包（6 函数 / 76 行）、`resolveCodexProxyEnvironment` 闭包（6 函数 / 79 行）。
