...
route 1（低耦合纯函数委托桩抽取）已于第十七块耗尽——TS 依赖图分析确认剩余候选均不再经济。此后交付走 route 3：把整个域的模块级状态与函数一并迁入 `createXxx(dependencies)` 工厂闭包，对齐 `preload/claude/index.cjs` 已有标准。route 3 共交付四个域：float（1,074 行）、archive（589 行）、environment（706 行）、以及随 float 同批的 window-size（95 行）。route 2（共享状态模块）未被采用。

**「按簇抽取」是 route 1 的正式口径，「闭包化改写」是 route 3 的正式口径，两者是先后阶段而非竞争方案**——这一点此前在计划文档里被记为「未选定」，与实施记录不符，本节予以统一。
...
| | 模块数 | 总行数 | 均值 | 中位 | 四分位 | 区间 |
| --- | ---: | ---: | ---: | ---: | --- | --- |
| `preload/codex/` | 31 | 5,945 | 192 | 117 | 85 / 117 / 192 | 41–1,074 |
| `preload/claude/` | 13 | 4,677 | 360 | 294 | 153 / 294 / 555 | 27–883 |

- **均值差距由 2.7 倍收窄到 1.9 倍**，主因是 route 3 产出的四个域闭包（1,074 / 706 / 589 / 292 行）填补了大模块段。
...
顶层函数 413 个（含 `codex` 字样 271 个），模块级可变绑定 147 个——绑定数相对草案基线 129 仍高 18 个，但已从 08-25 融合前的 166 个回落。草案点名的五大跨域缓存中，`codexActivityInventory`、`codexThreadActions`、`codexThreadTurnStatusCache` 仍在入口（按设计以引用或 getter 注入各闭包），`codexDesktopBridge`、`codexThreadGoalCache` 亦未迁出——route 3 消除的是域内状态，不是跨域共享状态。
