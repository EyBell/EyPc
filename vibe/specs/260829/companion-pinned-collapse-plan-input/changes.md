...
| 批次 | 本地提交 | 核心说明 |
| --- | --- | --- |
| Kernel | `5e9f473` | 精确当前交互先于终态，稳定 attention 与本地置顶顺序 |
| Codex 同步 | `6e94f52` | 保留当前 Plan 请求，清除已执行旧 Plan，防止旧 waiting 回弹 |
| Float | `3e523cb` | 置顶分组可折叠，隐藏任务不占编号，展开后立即重算 |
| 构建身份 | `7f9687e` | Main/Float 握手透传受管构建时间，Codex 页面展示实际宿主身份 |
| 文档 | `129b5f6` | 同步两组需求、当前真值、帮助、知识与防错证据 |
| 主目录产物 | 本条所属 `main` 提交 | main 重建受管身份并同步当前真值，构建目录不入 Git |
