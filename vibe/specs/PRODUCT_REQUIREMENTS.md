...
- The companion uses a real uTools child window and retains water-ball/compact-card skins, host-owned resize, automatic collapse and macOS all-Spaces behavior. In the expanded surface the six task Tabs are the first content; the old water/card, hidden, refresh, settings and close toolbar does not render there.
- Hovering the compact water ball directly expands the companion; no intermediate mini-detail appears. The center shows only the actual quota window whose reset is nearest, with transparent text background. A Weekly window adds a clear 5px full track and remaining-percent arc; an absent 5-hour or Weekly window is never fabricated. The expanded quota text likewise renders only server-returned windows.
- Host Snapshot V2 reads the local Codex App Server plus an allowlisted, read-only native project registry. The main `.codex-global-state.json` is authoritative and `.bak` is used only when the main file is missing/invalid. EyPc never writes either file and never scans Codex SQLite, LevelDB or conversation bodies.
...
- Conversation projection V3 exposes `全部 / 待输入 / 动态 / 已完成 / 已隐藏 / 项目`. The rolling-day boundary is inclusive. All task lists sort strictly by newest Turn `startedAt` descending then anonymous key; search filters the active Tab without changing order.
- `全部` contains every in-window unarchived task and marks locally hidden rows. `待输入` contains every waiting-input task, including locally hidden rows. `动态` internally orders `待输入 → 当前动态 → 已完成未查看`. `已隐藏` preserves actual state. `已完成` contains non-hidden, non-active rows whose newest Turn completed.
...
- Task and project aliases are display/search metadata only; original identity still governs order, ownership and archive. Native order is read-only. EyPc local pins can be reordered with `Alt+↑/↓`. “从 EyPc 移除” is reversible local suppression, never Codex deletion, and an upstream absent→present transition clears old suppression.
- First use defaults to `动态` while retaining `ongoing` as its stable persisted ID. Last Tab, project collapse, aliases, local pin order, local remove set, time window and display geometry persist. Search, selection, focus and confirmation do not persist. Storage uses hashed task keys and stable project fingerprints; raw IDs, paths, action aliases and task lists remain memory-only.
- Task rows always show fixed short-character actions `开 / 名 / 顶 / 隐（或显）/ 归`; project rows show `名 / 顶 / 归 / 移`. Every slot retains at least a 32px hit target and fixed position; unavailable actions remain visible as disabled slots. There are no colored action dots, icons, hover expansion or width animations; same-slot destructive confirmation changes only the character to `确`.
- The Codex floating child does not mount the main product Tooltip layer and never uses native `title`. The compact quota surface has no explanatory bubble. Task rows show an opaque detail card after 500ms; status controls and short-character actions show child-owned opaque explanations after 200ms. Full accessible names, pressed state and focus rings remain; other main-app surfaces continue using the shared Tooltip owner.
- Destructive operations require same-position confirmation within five seconds. The same button or shortcut executes the second step; Escape, outside click, Tab switch and timeout cancel. A row click opens the task; only the left selector, Ctrl/Cmd/Shift selection, or Space on the highlighted item changes selection. Adding with Space advances to the next item; removing stays in place. Search/Tab/refresh removes invisible selections.
...
- One item is highlighted per Tab. Arrow movement temporarily owns highlight until the pointer actually moves again. Right-click and Ctrl+right open the same full action drawer; its arrows move among actions and Enter/Ctrl+1…9 execute. Single-task actions may open or edit aliases; batch actions never expose operations that lack batch semantics.
- The isolated Codex shortcut domain owns arrows, Space, Enter, Ctrl+left/right, Delete, F2, Ctrl+P, Alt+up/down, Ctrl+F/R, Ctrl+1…9, F/Shift+F Quick Jump and Escape. Ordinary inputs retain native editing. Conflicts are rejected only inside the same feature domain and interaction layer, so other feature Tabs may reuse keys.
- App Server status notifications apply immediately and a single-flight lightweight inventory check normally runs every 200ms; after three consecutive failures it backs off to 1s. Structural changes schedule the complete verified snapshot path instead of pretending the activity delta is a complete inventory.
- The compact surface shows nonzero waiting-input in red text at upper-left, current-active at upper-right and completed-unread beneath it, capped at `99+`. A single waiting-input click opens that task; multiple tasks open the input Tab. Successfully opening a completed-unread task advances only that completion revision's local viewed watermark.
- A thread reported `active` by the connected App Server blocks archive. Single archive accepts only a short-lived action alias plus expected version/latest-Turn/source fingerprint, rereads identity/state/recency/Turn/native fingerprint, rejects active/inProgress/change, archives, then verifies absence from `archived=false` and presence in `archived=true`. Failure never optimistically removes a row.
...
- Three appearance presets and validated custom theme/water settings remain available. Required text, focus, progress and boundary contrast must remain valid, and reduced motion removes wave movement.
- Renderer-facing data never includes raw thread/Turn IDs, paths, cursors, Turn items, account/session IDs, tokens, native state objects or transcript content. Raw identity-to-anonymous-key/action-alias mapping exists only in preload memory.
...
- Renderer contracts remain behind Host Snapshot V2, Conversation projection V3 and platform-owned read/open/archive/float ports. A future Easy Agent adapter may replace provider/window ownership while keeping anonymous identities, six Tabs, local metadata and confirmation behavior stable.
