...
Requirement version: `2026-07-20.1`
...
- The companion uses a real uTools child window and retains water-ball/compact-card skins, host-owned resize, automatic collapse and macOS all-Spaces behavior. In the expanded surface the five task Tabs are the first content; the old water/card, hidden, refresh, settings and close toolbar does not render there.
...
- Conversation projection V3 exposes `全部 / 进行中 / 已隐藏 / 已完成 / 项目`. The rolling-day boundary is inclusive. All task lists sort strictly by newest Turn `startedAt` descending then anonymous key; search filters the active Tab without changing order.
- `全部` contains every in-window unarchived task and marks locally hidden rows. `进行中` contains non-hidden active/waiting/failed/interrupted/system-error/unknown rows. `已隐藏` preserves actual state. `已完成` contains non-hidden, non-active rows whose newest Turn completed.
...
- First use defaults to `进行中`. Last Tab, project collapse, aliases, local pin order, local remove set, time window and display geometry persist. Search, selection, focus and confirmation do not persist. Storage uses hashed task keys and stable project fingerprints; raw IDs, paths, action aliases and task lists remain memory-only.
...
- The Codex floating child has no visual Tooltip layer and no native `title` on the water ball, projects, tasks or their operations. Full accessible names, pressed state and focus rings remain; other main-app surfaces continue using the shared Tooltip owner.
- Destructive operations require same-position confirmation within five seconds. The same button or shortcut executes the second step; Escape, outside click, Tab switch and timeout cancel. Selection supports click, Ctrl/Cmd and Shift range; Space toggles the current task or a project's visible children without moving focus or scroll. Search/Tab/refresh removes invisible selections.
...
- The isolated Codex shortcut domain owns arrows, Space, Enter, Ctrl+left/right, Delete, F2, Ctrl+P, Alt+up/down, Ctrl+F/R, Ctrl+1…9 and Escape. Ordinary inputs retain native editing. Conflicts are rejected only inside the same feature domain and interaction layer, so other feature Tabs may reuse keys.
...
- One `codex.float.toggle` action owns companion visibility. Inside EyPc it defaults to `Command+Option+Q` on macOS or `Ctrl+Alt+Q` on Windows and remains available across Tabs and ordinary editors. A static `eypc-codex-toggle` uTools feature with `mainHide: true` exposes the same action for system-global binding; because uTools owns that binding, the configuration page opens the exact uTools shortcut setting instead of claiming or silently registering a global key.
- Renderer contracts remain behind Host Snapshot V2, Conversation projection V3 and platform-owned read/open/archive/float ports. A future Easy Agent adapter may replace provider/window ownership while keeping anonymous identities, five Tabs, local metadata and confirmation behavior stable.
