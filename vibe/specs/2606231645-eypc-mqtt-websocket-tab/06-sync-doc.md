...
- 2026-07-30 product-tooltip sync: MQTT icon/action controls in [MqttPage.vue](../../../src/pages/MqttPage.vue#L1) use `commandTooltip` / `plainTooltip` so [OperationTooltipLayer](../../../src/components/OperationTooltipLayer.vue#L1) receives always-on `data-operation-tooltip` and `data-operation-shortcut`. Selected style matches Codex product tips with label + shortcut kbd without requiring Ctrl-hint mode; avoided style is title-only tips that the layer strips, or shortcut attrs written only while `showShortcutHints` is true. Evidence label: `user-request`, from follow-up “还是没有悬浮的提示信息”.
...
| UI | Compact MQTT workbench, conditional selected-count/copy/save toolbar, record rows, config drawer, send-area draft popover, preview layer, product operation tips, and Ctrl-hint badges. | [MqttPage.vue](../../../src/pages/MqttPage.vue#L1), [MqttPublishRecordList.vue](../../../src/components/MqttPublishRecordList.vue#L1), [OperationTooltipLayer.vue](../../../src/components/OperationTooltipLayer.vue#L1), [app.css](../../../src/styles/app.css#L1) |
