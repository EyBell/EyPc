...
- Hook 状态由纯父 Turn reducer 归并：只有 `UserPromptSubmit` 开启 Turn；`SubagentStart/SubagentStop` 只更新活动水位。成功 `Stop` 关闭 Turn 后，同 Turn 的子代理、工具或 lifecycle 尾事件不得恢复 running；只有严格更新的新 Prompt Turn 可重新激活。Hook `StopFailure` 的当前合同见 Controlled [RAW-174](../../260810/1155-install-runtime-diagnostics/spec.md#L168)：它不得在同一 Turn 仍有 prompt/tool/permission 或 App live running 时保持「待继续」。SessionEnd 仍只关闭已观察的 open Turn。
