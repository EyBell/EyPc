...
updated: `2026-08-12`
status: `implementation-landed / RAW-032-full-automated-verified / artifact-ready / native-sidebar-unsupported / quota-host-verified / dev-plugin-reload-pending`
...
- RAW-030–032 / Companion RAW-160 已把 Claude current phase 因果优先移入 V4 Kernel，并把 state/membership/unread 监听移到进程原生文件回调：旧 inventory/cache 不得覆盖较新的 watcher/open-refresh terminal；hidden Main 下首事件不经过 JavaScript timer，已登记目标 1 秒 StatWatcher 只补漏，部分 JSON 保留最后可信 task，同值 unread 指纹不通知，phase、revision/time、unread、capabilities 原子接受。真实 Hook queue→Host→Kernel→Float applied 自动化通过正常 `≤250ms`、漏通知恢复 `≤1.25s`；当前影响矩阵为 `20/20` files、`547/547` tests，最终全仓 `83/83` files、`1325/1325` tests，typecheck、1871-module build、Runtime Identity、uTools validator、镜像/语法和文档审计通过。产物为 `host-7d964955afb146b4ee98 / renderer-3549884a1cbfed8c3984`；开发插件当前身份重载仍 pending，未执行真实归档。完整证据见 [RAW-160 verify](../../260810/1155-install-runtime-diagnostics/verify.md#L1)。
