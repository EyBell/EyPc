...
- Window Jump: [spec](260724/1527-window-jump-workbench/spec.md#L1) / [verify](260724/1527-window-jump-workbench/verify.md#L1). WJ-20 normalizes proven native members into one independent OS root target, keeps unrelated roots distinct, adds the Finder/Explorer virtual parent tree, and retains WJ-19 manual rebind after a root disappears. WJ-20.1 extends the single `windowTree` framework to row IDs, retained target/slot projection, action context, navigation and real-root-only selection, leaving Runtime as the side-effect adapter; conflicting old environment/Space/ArrowRight text is historical. Implementation/contracts are updated; tests, `vue-tsc`, build, uTools reload and native acceptance remain pending.
