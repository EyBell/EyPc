...
- Verification: RAW-129 基线保留 Bridge `51/51`、Controller `38/38`、RAW-128 状态链专项 `115/115`、Codex 状态矩阵 `168/168`、9 个 Codex 文件 `189/189`、完整仓库 `633/633`。RAW-130 已写入跨源水位实现和“旧 idle activity → 新 App Server active → read-state/refollow/连续 inventory → 真正后到 idle”回归契约，但按项目规则未执行测试、类型、正式 build、uTools runtime 或 preload 语法校验；canonical/public Codex 段与文档链接门禁见当前验证记录。
...
- Window Jump: [spec](260724/1527-window-jump-workbench/spec.md#L1) / [verify](260724/1527-window-jump-workbench/verify.md#L1). WJ-19 makes OS window-instance identity authoritative, removes title-based matching/rebind and requires explicit confirmation for every replacement candidate. Implementation/contracts are updated; tests/typecheck/build/uTools/native acceptance remain pending.
