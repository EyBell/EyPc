...
- Current task: Port group UI optimization follow-up is implemented through runtime-backed folders, folder `F2` rename, group/folder right-click action drawers, compact two-line group rows, one-line port rows, icon-only configurable `c-w` group toggle, input-safe `c-s-f` group search focus, no auto-highlight on group search or pane activation, direct `cr` search-history save with search blur and first-result highlight, history-candidate selection preserving search focus and caret end position, unchanged-search refocus without reopening history, `s-↑↓` explicit history candidate movement while plain `↑↓` keeps list navigation, group-row commands active after explicit highlight, readonly Shift group preview, `del` / `backspace` confirmed process kill, `c-del` / `c-backspace` force kill, global `s-esc` app hide, hold-`c` top-tab `c-s-num` and Ctrl-based search-box shortcut hints floated at the right edge without widening tabs or obscuring placeholders, bottom-bar `c-*` shortcuts summarized as system-aware hold-modifier text, physical-key-backed settings `c-a-s`, Ez short-chain shortcut display, and partitioned search history suggestions. Scope is tracked in [260617-eypc-port-group-ui-optimization/01-spec.md](260617-eypc-port-group-ui-optimization/01-spec.md#L1), with verification in [260617-eypc-port-group-ui-optimization/04-verify.md](260617-eypc-port-group-ui-optimization/04-verify.md#L1).
