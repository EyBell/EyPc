...
- Current requirement: RAW-116–126. `task-state-v3` carries activity and Turn provenance, keeps v2/legacy sources as degraded atomic packages, and never clears tasks solely for revision skew. Exact App Server active/Turn-started events establish `app-server-live`; same-revision exact started/inProgress may move forward, exact completed may use startedAt when completedAt is absent, and late native unread can wake one targeted latest-Turn reconciliation without becoming activity authority.
- State invariant: real activity patches open a new epoch; delta and full snapshot share one active-exit reducer. Unchanged pre-active terminal stays ongoing and cannot clear the baseline; accepted exact/targeted/corroborated terminal or explicit Desktop not-running closes the epoch. Read-state changes unread only.
...
- Verification: Bridge `41/41`, Controller `33/33`, Codex 状态矩阵 `153/153`; ordinary/recovered completion-unread、stale interrupted full-snapshot 隔离、晚到 unread 主动复核、same-revision restart 与真实 stop 均通过。`typecheck`、正式 build、uTools runtime、preload 语法和 canonical/public 镜像通过。全部 10 个 Codex 命名文件为 `186/191`，剩余 5 条是既有外观/模型合同问题，已在当前验证记录隔离。
