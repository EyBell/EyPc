...
- Current main line: Port management redesign plus quick-open file favorites: relevance search, user-defined port groups/folders, command-soul editing semantics, layered shortcut profiles, settings-page shortcut governance, dynamic feature tab maintenance, real process scan/kill fallback, compact help hints, metadata-only deletion, unified focus behavior, favorites virtual containers, explicit target initialization, split file/folder path picking, transient pick-review saving, duplicate path focusing, target quick-open, real folder one-level browsing, right-click action drawers, and uTools quick favorite entry.
- Current task: File management tab now prioritizes the quick-open favorites loop: users choose files with `Ctrl+O` or folders with `Ctrl+Shift+O`, review selected paths in non-persisted runtime state, edit name/parent/tags/color, save with `Ctrl+S` / `Ctrl+Enter`, cancel with `Escape`, then search/open/reveal/copy saved `name + absolute path + type` records. The full page keeps compact virtual containers, left-tree drag moves above / into / below, row-local menus/actions, on-demand directory rows, metadata-only removal, duplicate `kind + path` focusing, quick-open usage ranking, and `eypc-favorites-quick`. Scope is tracked in [2606201810-eypc-file-management-tab/01-spec.md](2606201810-eypc-file-management-tab/01-spec.md#L1), with verification in [2606201810-eypc-file-management-tab/04-verify.md](2606201810-eypc-file-management-tab/04-verify.md#L1).
