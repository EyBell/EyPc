...
- `RAW-045` (`active`): 状态与操作交互增量：App Server 状态通知与单飞轻量 `thread/list` 轮询共同形成待输入活动通道，正常周期 `200ms`，连续三次失败退避到 `1s`；结构变化才触发完整快照。页签顺序固定为 `全部 / 待输入 / 动态 / 已完成 / 已隐藏 / 项目`，`动态`页内部严格按`待输入 → 当前动态 → 已完成未查看`分组。水球左上以红色文字显示待输入数，右上显示当前动态数及已完成未查看数；待输入单项点击直接打开，多项点击/悬浮 200ms 打开待输入页。整行左键直接打开，只有左侧选择槽或高亮项 Space 才选中；Space 新增选择后自动下移，取消选择保持原位。任务行 500ms 显示不透明详情卡，状态图标和短字符按钮 200ms 显示不透明说明卡，不使用原生 `title` 或主应用 `OperationTooltipLayer`。同一页只有一个高亮项；方向键接管后鼠标静止不抢回，鼠标再次移动后恢复鼠标高亮。右键与 `Ctrl+→` 打开完整操作；单项与批量动作集合分离，抽屉打开后上下键只移动动作，Enter 或 `Ctrl+1…9` 执行。全页 `F/Shift+F` Quick Jump 覆盖可操作控件；新增全局 `codex.float.activate`/uTools 入口直接显示、展开并聚焦 Codex 卡片。成功打开完成未查看任务即推进该完成版本的已查看水位；项目折叠和自动收起反馈分别采用乐观更新与约 100ms 延迟。
...
| 状态与交互 | 200ms 轻量活动通道、三角标、常显短字符、延时不透明说明、Space 选中后下移、单/批量动作分离、右键/Quick Jump/全局激活 | `RAW-041`、`RAW-044`、`RAW-045` |
...
- Included: 早期 A1/B1/C1、D1/E1/F1 和 V2 演进事实；最新 RAW-035–045 对真实项目库存、完整分页、严格 Turn 时间、30 天窗口、六页签、200ms 活动通道、项目结构、本地元数据、真实额度/三状态角标、常显操作、延时说明、Space 下移/浮动批量栏、单批量抽屉、全局激活、双向归档与隐私边界的最终纠正；同时保留环境诊断、宿主兼容、自动收缩、主题与 macOS all-Spaces 基础合同。
