...
- `RAW-028` (`active`, `live-authority-refined-by-RAW-056`, `visible-status-refined-by-RAW-066`, `archive-capability-refined-by-RAW-068`): 任务快照升级为三种业务桶 `ongoing / completed-unread / completed`。原始 Host/Turn 继续保留 `interrupted` 供诊断与计时；RAW-066 后 Renderer 可见活动副状态改为 `active / ongoing / waiting-input / waiting-approval / failed / system-error / unknown`，上游 `interrupted` 在领域卡片投影时转换为 `ongoing`。RAW-068 进一步要求该投影同时控制归档能力，不再让原始 interrupted 单独形成可见动作差异。RAW-056 的 Desktop live 权威、最新 Turn 完成版本与未知降级合同继续有效。
...
- `RAW-042` (`active`, `active-guard-refined-by-RAW-068`): 单条真实归档须重读身份、状态、latest Turn、版本和项目指纹，拒绝 active/inProgress/变化证据，写入后同时确认从 `archived=false` 消失并在 `archived=true` 出现。RAW-068 后原始 interrupted 与投影后的 ongoing 同样拒绝；项目全部归档忽略 30 天窗口，跳过 active/inProgress/interrupted，20 条一批、并发 2、逐项双向验证并保留部分失败；批量真实写入只在另行授权临时项目时执行。
...
- `RAW-065` (`active`, `supersedes-RAW-063-weekly-ring-removal`): 删除水球表面及宿主按钮交互态的普通装饰圆环，包括 `2px inset`、静态 border、inset outline、装饰 shell、根容器整圆背景、同尺寸外发光与外部圆形 focus outline；键盘焦点改由中央读数的非圆形下划线提示。恢复仅在存在 Weekly 读数时渲染的数据进度环。进度环表示同一额度池的 Weekly 剩余百分比，支持连续圆环与固定 20 段模式，并恢复粗细、额度状态/自定义进度色、轨道色与光晕设置和 `3:1` 对比度校验；不恢复普通轮廓透明度入口，历史 `shellOpacity` 仅作持久化兼容。5 小时 + Weekly、Weekly-only 与含 Weekly 的 Spark 都显示数据环；无 Weekly 时不渲染任何外圈。
...
| 紧凑角标点击 | 待输入与完成未读非零时均直开实际计数集合中展示排序第一条；进行中仍只展开；角标直接消费任务级 2 秒稳定投影，不再叠加独立延迟；打开不改未读/隐藏/页签 | `RAW-050`、`RAW-058`、`RAW-063`、`RAW-067`、`RAW-069` |
...
| 原生归档 | 单条和项目逐项双向验证；desktop-live active、inProgress 与原始 interrupted 均拒绝/跳过；投影 ongoing 的固定归档槽稳定禁用；部分失败保留；验证后通知桌面端即时刷新 | `RAW-042`、`RAW-056`、`RAW-068` |
