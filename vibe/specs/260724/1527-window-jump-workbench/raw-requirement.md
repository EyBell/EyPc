...
- On macOS, a Core Graphics window ID is an inventory/session reference, not a System Events `AXWindowNumber`. When the private Accessibility symbol is available, activation must first revalidate the same PID/CG ID and compare titles only inside the Core Graphics source, then map each owning-process `AXUIElement` through `_AXUIElementGetWindow` and require exactly one element whose returned CG ID equals the selected reference. `AXTitle` must not be compared with the saved CG title on this exact path. Normalized AX title/fresh ordinal remains a compatibility fallback only when exact mapping is unavailable; the bridge must never assume numeric identity without the mapping call, interpolate a user title into shell/JXA source, or select an ambiguous target arbitrarily.
...
25. An unchanged macOS CG-backed target whose AX title differs from its CG title still passes exact CG-ID-to-AX mapping and focus verification. `target-title-changed` is emitted only when the current title from the same CG record explicitly differs; unavailable title evidence produces a non-title verification failure.
