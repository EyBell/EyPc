...
Provider enums are evidence, not automatically product vocabulary or action capability. Normalize the complete product state once in Kernel—including Plan lifecycle and destructive-action availability—and always include every confirmed root/Side bead in the parent scope。Within that scope，waiting and causally newer active retain their existing priority；for the completion-facing states，running outranks completed-unread and completed，and latent unread cannot create a second public group/count while active。Ordinary interruption needs exact branch-idle confirmation，and an unexecuted Plan interruption additionally needs targeted no-newer-Turn/activity/pending proof。Transport failure or conflict preserves the last stable state as verifying。Every UI/count/action surface consumes the same package selector，and every open uses the parent identity even when a Side Chat supplied the visible state。A stopped archive must still be revalidated at mutation time；do not fix labels、counts、navigation or actions with scattered Controller/Renderer branches。

## Alternative Route

- Status: `verified` for the RAW-164 focused automation；current rebuilt-host acceptance remains pending.
...
- Ordered steps: preserve raw status/unread → classify inventory/Desktop topology privately → reduce each branch → aggregate every confirmed bead → derive visible state/capability once → open only parent → revalidate mutation in Host.
- Verification: main completed-read or completed-unread + Side running becomes running；no active bead + any unread becomes completed-unread；all read becomes completed；active/unread groups and counts stay mutually exclusive；a Side never becomes a public row or Deep-Link target。Bridge+Kernel+Runtime Diagnostics `189/189`、canonical/public mirrors、typecheck and production/runtime validation pass，while the matching rebuilt uTools acceptance remains pending。
