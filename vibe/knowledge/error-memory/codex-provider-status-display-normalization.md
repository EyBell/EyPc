...
Provider enums are evidence, not automatically product vocabulary or action capability. Normalize the complete product state once in Kernel—including Plan lifecycle and destructive-action availability—but choose the parent scope before applying cross-branch priority。Main owns phase/unread until it is exact completed-read；only then may Side Chat evidence participate。Within the selected scope，waiting and causally newer active retain their existing priority；ordinary interruption needs exact branch-idle confirmation，and an unexecuted Plan interruption additionally needs targeted no-newer-Turn/activity/pending proof。Transport failure or conflict preserves the last stable state as verifying。Every UI/count/action surface consumes the same package selector，and every open uses the parent identity even when a Side Chat supplied the visible state。A stopped archive must still be revalidated at mutation time；do not fix labels、counts、navigation or actions with scattered Controller/Renderer branches。

## Alternative Route

- Status: `verified` for the RAW-163 focused automation；current rebuilt-host acceptance remains pending.
...
- Ordered steps: preserve raw status/unread → reduce each branch → identify main → select main-only unless main completed-read → apply existing cross-branch priority only inside that scope → derive visible state/capability once → open only parent → revalidate mutation in Host.
- Verification: main completed-read + Side running becomes running；main completed-read + Side completed-unread becomes completed-unread；main completed-unread/interrupted/waiting/running remains main-owned despite a conflicting Side；canonical groups/counts converge；an active Side never becomes the Deep-Link target。Bridge+Kernel `177/177`、canonical/public mirrors、typecheck and production/runtime validation pass，while the matching rebuilt uTools acceptance remains pending。
