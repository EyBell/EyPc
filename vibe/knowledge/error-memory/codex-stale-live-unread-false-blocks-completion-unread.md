...
RAW-164 adds the missing parent-scope and runtime-version boundaries. An inventory-listed Side Chat must carry its own Turn-bound read evidence inside the private root relation instead of being published as a second top-level row or borrowing only the parent Turn. While any bead is active, lower-priority unread stays latent and cannot increment the public unread group/count；after activity ends it may surface as completed-unread. A successful parent open binds the current root/known Side Turns，and older unread true/false、full snapshots、duplicate Goal notifications or same-Turn metadata enrichment cannot revoke it. Because the reported drift came from an older Host，`runtime-identity-handshake=host-loaded` is required before judging the fix.

## Evidence

- [preload/index.js](../../../preload/index.js#L1) resolves unread directionally: exact Desktop or successful-open session event first, then current snapshot false, then current parseable native-set membership/nonmembership over snapshot true, then source-unavailable fallback. A later native-set parse whose membership line changed and no longer lists the thread drops a stale exact unread event, so Codex 侧栏无小点时 EyPc 不得继续标未读。
...
Do not impose a symmetric total order on reconnect read evidence. Exact Desktop events and confirmed Host-open facts are strongest. A current refollow snapshot false must repair stale persisted true after a missed disconnected event; native nonmembership must defeat snapshot true. A transient native read failure must not erase the last successful native decision or create a one-frame fallback; retain it only inside the current Bridge session and let fresh exact/native evidence supersede it. Native-file changes remain read-state only, and a local acknowledgement is session-only, success-gated, bound to each concrete internal root/known-Side Turn and never persisted. Timestamp enrichment is not Turn identity. The first available true observation and a later false/unknown→true change may trigger bounded evidence collection when completion is unknown, but unread cannot itself change Turn/Activity；while any bead is active it remains latent rather than creating a second public unread group. A genuinely new completion epoch must remove the prior false so it can become unread.

## Latest Applicable Implementation

[preload/index.js](../../../preload/index.js#L1) owns unread arbitration, inventory/Desktop Side topology, successful-open acknowledgement and completion-epoch cleanup；[task-kernel.cjs](../../../preload/companion/task-kernel.cjs#L1) owns latent unread versus active/completed grouping. [codexController.ts](../../../src/runtime/codexController.ts#L1) keeps every card/counter/shortcut route on the shared `openThread` action. [codexAppServerBridge.test.ts](../../../tests/platform/codexAppServerBridge.test.ts#L1)、[companionTaskKernel.test.ts](../../../tests/platform/companionTaskKernel.test.ts#L1) and [codexController.test.ts](../../../tests/runtime/codexController.test.ts#L1) own the bridge/kernel/open regressions. The controlled acceptance record is [verify.md](../../specs/260810/1155-install-runtime-diagnostics/verify.md#L1).
...
| 2026-08-12 | RAW-164 Side/Cloud unread convergence | Completed-unread drifted and sending a new description caused a sudden refresh；inventory-listed Side Chat also needed root-scoped read handling | Judged an old Host as current，kept child outside the inventory topology and lacked final Goal-unread/read-ack rollback coverage | Add loaded-identity handshake，private inventory root/Side relation，per-Turn root/Side acknowledgement，latent unread during active and stale snapshot/duplicate Goal rejection | focused `189/189`、type/build/runtime validation pass；real host dual-snapshot pending |
| 2026-09-14 | CX 侧栏无小点仍未读 | Exact unread event 残留，原生集合已不含该线程 | 精确未读事件压过后来的 native non-membership | 用户确认 Codex 无小点 | 原生 unreadLine 变化且集合不含该 id 时丢弃过期 exact unread | pending-host |
...
- Verification: Bridge two-direction reconnect/read-state, successful/failed/offline open, v11/v2 and v6/v1, inventory/Desktop Side Chat aggregation, active-latent-unread, stale snapshot/duplicate Goal rollback and Controller state sequence pass; preload canonical/public mirror, syntax, typecheck and production validator pass.
