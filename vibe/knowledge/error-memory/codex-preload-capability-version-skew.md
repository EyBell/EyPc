...
- RAW-160 hard gate: [eypcPlatform.ts](../../../src/platform/eypcPlatform.ts#L1)、[runtimeIdentity.test.ts](../../../tests/platform/runtimeIdentity.test.ts#L1) and [verify.md](../../specs/260810/1155-install-runtime-diagnostics/verify.md#L1).

## Correct Detection Order

1. Distinguish a true no-host browser from a desktop host that already exposes the Codex snapshot bridge.
2. If the snapshot bridge is also absent, remain unsupported. If only a non-task additive inspection method is absent, infer no more than the macOS/Windows category from browser host metadata and show a neutral“等待连接验证”state；a missing V4 task Kernel/facade is different and must be `reload-required`.
...
Every additive preload port must define a mixed-version behavior separately from the no-host browser fallback. Capability absence is not automatically platform failure, and a successful lower-level round-trip must reconcile every dependent readiness field as one state transition. Async host reads require a generation guard across disable/re-enable and disposal. Stateful task Kernel/package/actions methods are not optional additive diagnostics：their absence or identity mismatch must stop task actions with `reload-required` and cannot reuse a legacy Controller/Renderer reducer.
...
[eypcPlatform.ts](../../../src/platform/eypcPlatform.ts#L1) still recognizes a legacy additive diagnostic bridge without prematurely verifying the CLI，but it requires the V4 task Kernel/facade whenever Runtime Identity claims the current artifact。A missing/incompatible task bridge becomes `reload-required / kernel-missing`；[codexController.ts](../../../src/runtime/codexController.ts#L1) has no legacy task selector/action fallback。Task-state semantic skew is detailed in [codex-task-state-version-skew-must-degrade-atomically.md](codex-task-state-version-skew-must-degrade-atomically.md#L1)。[CodexPage.vue](../../../src/pages/CodexPage.vue#L1) keeps only genuinely additive pre-success diagnostics neutral。

## Alternative Route

- Status: `verified`.
- Preconditions: a desktop host exposes the existing snapshot bridge, but a non-task additive diagnostic method may be absent because the host preload has not reloaded；Kernel/package/actions identity remains exact.
...
| 2026-08-11 | RAW-160 no-fallback ownership | A current Renderer/identity could otherwise call missing V4 Kernel methods and silently revive old Controller selectors | Treat task Kernel absence like an optional additive port | Convert missing/incompatible V4 facade to `reload-required / kernel-missing` and remove Controller task fallback | focused/full automation and artifact identity verified；real host reload pending |
