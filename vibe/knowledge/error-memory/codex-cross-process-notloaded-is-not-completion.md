...
The first correction removed false completion but stopped at inventory-only `notLoaded`, so a fresh separate App Server lacked persisted outcome evidence. Current App Server protocol also exposes status-only `thread/turns/list(itemsView=notLoaded)`: it cannot recover foreign live input/running state, but the latest Turn's `completed`, `failed` or `interrupted` value is usable raw outcome evidence without loading turn items. Only `completed` forms the completed business bucket; RAW-089 keeps every other raw outcome as visible ongoing. Latest Turn `startedAt`, not thread recency, is the only safe “last question time”.
...
3. Preserve latest Turn outcomes as raw evidence: completed forms a completion revision; failed/interrupted/systemError remain diagnostic-only and project to ongoing.
4. Treat bare `notLoaded`, missing metadata and timeout as ongoing; never infer active, completion or failure from recency or elapsed time, and never expose “unknown” as a task product state.
...
Never derive live state, completion or last-question time from inventory visibility, elapsed time or recency. A provider adapter must distinguish exact thread-active evidence, persisted Turn outcomes and inventory-only metadata. Only a latest completed Turn with valid `startedAt` (and optional `completedAt`) forms completion; failed/interrupted/systemError/unknown stay ongoing. Exact foreign input/approval/running still requires a shared live authority.

## Latest Applicable Implementation

[preload/index.js](../../../preload/index.js#L1) enriches every valid recent unarchived row with bounded, cached status-only latest-Turn reads, performs an additional bounded one-task read after Desktop active exit, rejects an unchanged pre-active completed revision, and never forwards items/raw IDs. [codex.ts](../../../src/domain/codex.ts#L1) owns completed-only V2 buckets, conservative ongoing projection, viewed/hidden migration and the shared comparator. [codexAppServerBridge.test.ts](../../../tests/platform/codexAppServerBridge.test.ts#L1), [codex.test.ts](../../../tests/domain/codex.test.ts#L1) and [codexController.test.ts](../../../tests/runtime/codexController.test.ts#L1) contain the current privacy, fallback, sorting, migration and archive contracts.

## Alternative Route

- Status: `verified` for the cross-process evidence baseline; RAW-089 visible-ongoing and targeted-transition refinements are `reported/unverified`.
...
- Ordered steps: preserve exact active flags; classify exact active and persisted latest-Turn results independently; keep bare notLoaded/timeout ongoing; use latest Turn `startedAt` for question time; enrich all current rows with bounded concurrency/cache; target only the changed task after active exit; project completed-only V2 buckets; migrate only viewed/hidden watermarks.
...
- Fallback: when latest-Turn metadata is absent, malformed, unsupported or slow, keep that row visible as “进行中” and place missing question time after exact peers; never infer from recency.
...
| 2026-07-26 | RAW-089 conservative real-time transition | User required near-real-time completion while every abnormal/unconfirmed state remains ongoing | Generic two-second debounce, full-scan completion discovery and visible unknown/exception states | Three-minute steady-state observation plus source/contracts | Immediate ongoing, bounded one-task latest-Turn confirmation, stale prior-completion rejection and completed-only exit | reported/unverified |
| 2026-07-27 | RAW-099 connected-bridge settle | User saw 2 ongoing while only 1 Desktop task was active | Non-active failed/interrupted/notLoaded stayed ongoing even when live bridge was connected | Domain projection + user count mismatch | When bridge is connected, settle non-active terminal/empty outcomes to stopped immediately; keep inProgress ongoing only for shadow lag; keep conservative ongoing when bridge is unavailable | candidate; source/tests updated, host acceptance pending |
