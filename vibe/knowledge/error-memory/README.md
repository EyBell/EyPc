...
| Inventory membership | New rows require verified admission; one missing snapshot is not deletion; only a mapped explicit archive may remove one anonymous key immediately | [inventory dropout](codex-inventory-dropout-is-not-task-deletion.md#L1) | [explicit archive fast path](codex-explicit-archive-event-bypasses-inventory-quarantine.md#L1) |
| Live stream ownership and ordering | Ignore well-formed private patch content while advancing revision; task-selection unfollow is not client loss; distinguish initial snapshot from real activity patch; accepted terminal closes and clears its activity-exit baseline | [active/terminal ordering](codex-stale-live-active-needs-completion-order.md#L1) | [private patch continuity](codex-unobserved-private-patch-must-not-resubscribe.md#L1) · [task-switch follow continuity](codex-task-switch-unfollow-must-not-drop-live-shadow.md#L1) |
...
- [macos-cg-ax-window-identity-mismatch.md](macos-cg-ax-window-identity-mismatch.md#L1): verified project pointer that `_AXUIElementGetWindow` must uniquely map the selected CG ID and `AXFocusedWindow` must read back that exact element; title/ordinal is compatibility fallback only.
- [utools-macos-cross-api-window-title-mismatch.md](utools-macos-cross-api-window-title-mismatch.md#L1): candidate thin pointer for WJ-18: compare saved/current titles only inside Core Graphics, never use `AXTitle` disagreement to claim an exact CG target changed, and keep unavailable evidence as a generic block.
...
- [utools-window-target-auto-rebind-after-restart.md](utools-window-target-auto-rebind-after-restart.md#L1): candidate pointer for restart-safe logical-window replacement: exact app identity, complete-inventory high-confidence uniqueness, success-only title/reference learning, and manual fallback for partial/weak/ambiguous siblings.
