...
- [codex-coupled-color-editor-atomicity.md](codex-coupled-color-editor-atomicity.md#L1): coupled surface/foreground colors require actual linked boards, visible valid gamut, one local draft, reversible real-target preview and one atomic commit; independent popups, adjacent sliders or editor controls inside the target float cannot satisfy the contract.
...
- [codex-selection-state-needs-structural-contrast.md](codex-selection-state-needs-structural-contrast.md#L1): candidate rule that dense-list selection needs a named mode, broad stable selector, preserved status identity, nonmember de-emphasis and deterministic row/button key ownership.
