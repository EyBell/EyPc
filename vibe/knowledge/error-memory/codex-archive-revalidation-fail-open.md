...
The preflight used an incomplete denylist and a completion-only request shape. It did not prove that reread identity, recency, request revision and newest-Turn evidence still described the exact row shown to the user. Extending archive to failed/interrupted/systemError/unknown makes “not explicitly active” even less sufficient: the operation must validate the evidence class it was asked to archive and cancel on `active`, `inProgress`, new versions or malformed shapes.
...
4. Independently read the newest status-only Turn. Reject `active`/`inProgress`; require exact completed revision for completed evidence and exact failed/interrupted status for terminal evidence. Unknown evidence may proceed only with a structurally valid non-active reread and the UI's explicit cross-process warning.
...
Destructive revalidation must prove the exact current non-active row and its evidence class. Never infer safety from a stale UI card, “not explicitly active,” falsy fields or schema-default values. Identity, status, recency, revision, response shape and newest-Turn evidence are mandatory; unknown-state archive additionally requires an explicit warning and must still fail closed on structural uncertainty.

## Alternative Route

- Status: `verified`.
- Preconditions: the App Server exposes `thread/read`, status-only newest-Turn read and `thread/archive`; the UI has an exact non-active revision, evidence enum and live alias.
- Ordered steps: require second confirmation; show a cross-process warning for unknown; validate alias/request; reread identity/status/recency/response shape and latest Turn; reject active/new-version/evidence mismatch; archive; remove only on success; preserve the row on failure.
