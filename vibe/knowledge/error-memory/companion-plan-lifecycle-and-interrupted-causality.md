...
2. Resolve every exact current input/approval/Plan interaction instance before terminal state and even while `activityPhase` is running，without using unread=true as an eligibility gate；a completed-read task may still be waiting-input. Confirm Cursor evidence includes `kind: observation.interactionKind`.
...
- Current requirement/acceptance: [RAW-207](../../../vibe/specs/260903/same-turn-waiting-input/spec.md#L1)；[RAW-189](../../../vibe/specs/260829/companion-pinned-collapse-plan-input/spec.md#L1)；[RAW-179 V7](../../../vibe/specs/260824/eypc-v7-global-refactor/spec.md#L1) and RAW-160/176 remain foundations.
...
| 2026-09-03 | RAW-207 | Codex Questions / Cursor AskQuestion during an inProgress Turn stayed in 进行中 instead of 待输入 | Kernel returned immediately when `activityPhase === running`；aggregate live priority ranked running above waiting；Cursor observation had `user-input` but the synthetic bundle omitted `kind` | Open exact current interaction outranks a still-running Turn；live member priority is waiting-approval > waiting-input > running；Cursor forwards `interactionKind`. Claude AskUserQuestion and CodexHost `attention=input` already reached waiting and stay unchanged | focused Kernel source+behavior regression；uTools Codex Questions host-verified；Cursor AskQuestion not named this round |
