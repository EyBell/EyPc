...
- RAW-146 found the same split inside the complete input-required exception: Controller used the raw first item while Float/cycle used pinned-first display order. [codex.ts](../../../src/domain/codex.ts#L1) now owns `orderCodexTasksForDisplay`; [codexController.ts](../../../src/runtime/codexController.ts#L1) and [FloatApp.vue](../../../src/FloatApp.vue#L1) both call it, and the compact action text exposes the same first-item behavior to sighted and assistive-technology users.
...
3. For every first-item action, compare the ordered key list—not only the set or count—and require a fixture whose source first item is unpinned while a later item is pinned.
...
5. Check semantic exceptions separately: complete waiting-input/unread collections, dedicated completed-unread open-first action and explicitly EyPc-local pinned fallback must not be accidentally collapsed into active-card eligibility.
...
Define final product eligibility once as a stateless pure projection over the stabilized Controller snapshot. Cards, compact counts, summaries, accessibility text, previews and generic navigation actions must consume the relevant arrays from that projection rather than repeat time/hidden/state filtering. A shared set must also use one named display-order function before any first-item action; never read raw `[0]` in one consumer while another applies pinned-first ordering. Keep explicit semantic exceptions named and narrow: waiting-input and completed-unread may retain complete hidden-inclusive counts and dedicated first-item actions; previous/next may use complete waiting-input plus the shared recent active group; only an explicit EyPc-local non-stopped pin may bypass an empty ordinary action pool. Action hints and ARIA must describe the exact click/keyboard effect, and tests must derive expected copy from the current requirement rather than preserve implementation text. Pagination or source completeness must change the source contract, never be approximated by a downstream cap. Do not add a consumer-local timer or debounce to repair projection drift.
...
- `orderCodexTasksForDisplay` is the sole pinned-first/stable order for Float and Controller first-item/cycle consumers.
- Float cards, badges, summaries/ARIA and settings preview share the projection; input/unread hints explicitly say “打开第一条”.
...
- Verification: cards and active badge have identical keys/counts across jitter; an unpinned task just beyond the configured hours is absent from both active display and previous/next; editing the hours immediately changes both through the same package; a later pinned waiting item beats the raw source first item in both compact/global direct-open; local pin restores only the documented empty-pool fallback; completed-unread stays outside generic cycling; search never changes compact counts; hover/focus/ARIA names the click action.
