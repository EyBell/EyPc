...
3. For every first-item or progressive action, compare the ordered state-instance list—not only the set or count. Every status group and generic cycle tier uses latest-question-first; dedicated attention actions additionally retain Controller-owned unseen-instance progress.
...
- [companionTaskPackage.ts](../../../src/domain/companionTaskPackage.ts#L1) owns the V3 atomic projection across card, tab, project, group, badge, action, focus and cycle consumers; [codexPresentation.ts](../../../src/domain/codexPresentation.ts#L1) remains its configured recent-hours/status grouping input.
...
- Every status group and generic cycle tier orders by latest question, independent of pins and Provider grouping. Dedicated waiting-input and completed-unread actions retain their separate status-instance progress.
...
- Kernel stabilization remains upstream; the shared projection is stateless, adds no Renderer clock and is applied directly by Main and Float. New Codex membership is immediately renderable as a minimal card and then receives narrow metadata hydration rather than waiting for broad refresh.
...
- Status: `verified`; source and focused V3 regressions cover 240 tasks across three protocol pages, including consumer checks for keys 41、101 and 201. Final full-suite/build evidence and real-host interaction remain separate acceptance gates, not conditions for recalling this projection rule.
