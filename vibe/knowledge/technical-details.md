...
- Domain source: [src/domain/quickJump.ts](../../src/domain/quickJump.ts#L1); layout source: [src/domain/quickJumpLayout.ts](../../src/domain/quickJumpLayout.ts#L1); overlay source: [src/components/QuickJumpLayer.vue](../../src/components/QuickJumpLayer.vue#L1); App integration: [src/App.vue](../../src/App.vue#L1).
...
- Overlay markers are placed through [src/domain/quickJumpLayout.ts](../../src/domain/quickJumpLayout.ts#L1): title anchors stay near the title, large buttons/text controls/rows can use internal top/edge candidate positions, and every candidate is viewport-clamped and scored against previously placed badges to reduce overlap.
- Quick Jump badges intentionally may cover the original button and use transparent red-purple themed text without a badge background, border, or shadow. Ordinary markers alternate rose, purple, and fuchsia at 14px; active markers use deeper purple with a light underline. Badge text is not max-width clipped.
...
- App visibility filtering intersects each target with scroll/hidden/clip ancestors and skips targets whose visible area is under 6px in either dimension. This keeps offscreen or clipped action buttons from receiving markers.
