...
| [actions.ts](../../../../src/runtime/feature/ports/actions.ts#L1) | 该 Tab 前缀的 `register` / `registerHandler`；实现仍闭包同一份 Runtime state |
...
- 动作：[appRuntime.ts](../../../../src/runtime/appRuntime.ts#L8917) 组 [FeatureActionHostV7](../../../../src/runtime/feature/featureActionHost.ts#L4) 袋子后遍历各包 `registerActions`。Shell 只留 `app.hide` / `runtime.logs.*` / `quickJump.*` / `tab.select.*` / `settings.open` / `search.focus` / `confirm.*`。实现仍闭包同一份 state，未拆 AppState。
...
- **动作按包登记，实现仍在 Runtime 闭包**。[appRuntime.ts](../../../../src/runtime/appRuntime.ts#L8917) 的 `registerActions` 只组袋子并调用各包；切 Tab / 搜焦点副作用见 [feature-action-extract](../../../specs/260904/feature-action-extract/task-card.md#L1) 第 2–4 刀。
