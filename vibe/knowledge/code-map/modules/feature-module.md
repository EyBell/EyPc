...
| [pageBind.ts](../../../../src/runtime/feature/ports/pageBind.ts#L1) | 页面组件 + props；六包 `on` 只留 `dispatch`。[feature-bindpage-dispatch](../../../specs/260905/feature-bindpage-dispatch/task-card.md#L1) |
...
- **动作按包登记，实现仍在 Runtime 闭包**。[appRuntime.ts](../../../../src/runtime/appRuntime.ts#L8906) 的 `registerActions` 只组袋子并调用各包。切 Tab、全局搜焦点、ports/windows 壳 DOM 对焦与 CommandHints 文案已迁到模块可选贡献。`bindPage` 事件：六包已 dispatch-only；`FeaturePageHostV7` 只有 `dispatch`，包内适配器用具名交叉。[feature-bindpage-dispatch](../../../specs/260905/feature-bindpage-dispatch/task-card.md#L1)。
