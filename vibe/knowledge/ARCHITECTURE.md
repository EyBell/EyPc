...
- [src/FloatApp.vue](../../src/FloatApp.vue#L1) renders six tabs as the first expanded content, then unified search, actual ordinary/Spark quota text and the task/project list. It owns selection-first rows, project `＋`, one mouse/keyboard-arbitrated highlight, Space/range selection, same-position confirmation, the top/bottom avoiding batch toolbar, synchronized complete drawers, pure Shift preview, composer, local transient layers and filtered Quick Jump. [src/runtime/keybinding/keybindingRuntime.ts](../../src/runtime/keybinding/keybindingRuntime.ts#L1) owns the Codex profile including rebindable `Ctrl+T`; the child applies the same `when`/layer precedence without delegating DOM focus to the parent. [preload/float.js](../../preload/float.js#L1) transports effective bindings, narrow expansion/activation/drag/resize messages and a dedicated request/result channel for transient new-thread creation.
- [CodexWaterBall.vue](../../src/components/CodexWaterBall.vue#L1) receives the quota projection from [codexPresentation.ts](../../src/domain/codexPresentation.ts#L1): ordinary 5-hour positive quota, ordinary weekly positive quota, then the highest positive Spark pool. [FloatApp.vue](../../src/FloatApp.vue#L1) owns water-mode compact hit routing: the upper half never hover-expands so counters remain directly clickable, while pointer entry/movement in the lower half expands immediately; explicit click/keyboard remains available and touch hover is ignored. The compact-card skin retains surface hover expansion without routing counter events through that surface. Spark adds an `S` above the percentage and uses its own weekly ring; an absent window is never fabricated or treated as zero. Existing appearance and reduced-motion infrastructure remains in [codexAppearance.ts](../../src/domain/codexAppearance.ts#L1).
...
- The floating child is host-owned and auto-collapses about `100ms` after pointer/focus departure unless focus, resize, composer or confirmation owns interaction. It has no Pin/manual-collapse control, does not mount the main app's `OperationTooltipLayer`, never uses native `title`, and has no ordinary hover detail layer. Complete explanations live in the action drawer, pure Shift preview and ARIA. On macOS the parent reapplies always-on-top and all ordinary/full-screen Space visibility after creation, display migration and geometry rebuild; only privacy-safe capability booleans cross to settings diagnostics.
- [FloatApp.vue](../../src/FloatApp.vue#L1) keeps only project `＋` as a direct row action and routes all task/project operations through the synchronized drawer. Two or more visible selections create an absolutely positioned batch toolbar inside the list stage; placement is recomputed against the focused or last-selected row and switches to the opposite top/bottom edge without reordering or translating task rows.
