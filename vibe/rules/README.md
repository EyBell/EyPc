...
- Companion providers are separate modules behind one interface plus an aggregation layer. `src/domain/companionProvider.ts` is the lowest layer and must not import `codex.ts`. Each provider is independently enableable; with only Codex enabled the companion must stay byte-identical to the pre-multi-provider release, including empty-state wording. A provider failure degrades that provider alone.
- Claude native state is read-only with exactly one exception: hook and status-line registration in `~/.claude/settings.json`, which requires action-time user confirmation, must be idempotent and marked, must preserve the user's existing hooks and status line, and must uninstall cleanly. Conversation content, tool arguments and credential values must never enter an observation, a diagnostic or a persisted file.
...
| `EYPC-COMPANION-PROVIDER-001` | multi-provider companion; 2026-08-05 claude-companion-provider | [companionProvider.ts](../../src/domain/companionProvider.ts#L1) · [companionAggregate.ts](../../src/domain/companionAggregate.ts#L1) · [claude.ts](../../src/domain/claude.ts#L1) · [spec](../specs/260805/1150-claude-companion-provider/spec.md#L1) · [verify](../specs/260805/1150-claude-companion-provider/verify.md#L1) | active; Codex-only stays byte-identical, providers stay isolated, RAW-146 order contract superseded by shared comparator + dual projection |
