...
- 新增: 设置 → 功能开关可查看各功能操作说明。
- 修复: MQTT 默认宽度下可切换上下 / 左右收发布局。

## 窗口跳转用户版本更新日志（2026-07-26）

- 新增: 可选窗口跳转工作台（搜索 / 收藏 / 别名）；默认关闭。
- 新增: 窗口槽 1–10，绑定后可直接跳到目标窗口，找不到时再打开工作台。
- 体验: 支持多选与批量操作；关闭窗口失败时可确认强制结束进程。

## Codex 全浮球用户版本更新日志（2026-07-23）

- 新增: Codex 悬浮球，展示 5 小时 / Weekly 额度与任务状态。
- 新增: 任务工作台，按进行中、已完成、已隐藏、项目查看与操作。
- 体验: 待输入 / 完成未读角标可一键打开对应任务；支持快速跳转与多选批量操作。
- 体验: 任务完成状态更稳，减少闪烁；进行中任务不可归档。
