...
- 插件内默认：`Ctrl+Alt+Q`（macOS 常为 `Command+Option+Q`）切换悬浮球；`Ctrl+Alt+Enter` 显示并展开卡片；`Ctrl+Alt+K` 进入快速任务查看。从快捷键、展开卡片或角标打开某条对话后，列表高亮会跟到这一条。「上一个 / 下一个」在所有已启用来源的状态候选中循环：待输入／待确认 → Plan 待继续 → 进行中 → 已完成未读 → 可继续的置顶任务，同一层按最近提问时间排序。打开候选中的任务会接着它走；点击已完成已读、普通置顶泊位等候选外的任务，仍保留原循环位置，不会把快捷键切成已完成组内循环。
...
任务来源缩写为 `OR`，悬停为「归属 Orca」。项目名取仓库显示名，不显示本机绝对路径；抬头优先使用 Orca 标签标题，工作中 spinner 不替代主题。相位跟随原生状态及终端工作帧；Grok 进入等待回复时即可显示进行中。Claude 主轮次结束后若仍显示后台监控，按已完成而不是进行中。排序和行上时间以这一轮提问时间为准，回复继续刷新不会把卡片往上抬。
