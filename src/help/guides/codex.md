...
- 通过 CodexHost 拉起的额外进程（Pi、Claude Code 等）会出现在同一张任务列表里。Host 仍在跑当前回合时显示「进行中」；回合结束后进入「已完成未读」，直到你在 Codex 里打开过该会话。官方未读小点管不到这些会话，EyPc 跟的是 Host 自己的未读
