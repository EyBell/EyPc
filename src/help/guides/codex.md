...
- **版本提示：** 当前任务状态语义为 `task-state-v6`。uTools 若仍保留旧版任务桥，EyPc 会保留当前可用的任务卡片与三个角标，在状态摘要中标记“状态已保留，建议重载”；旧版缺失权限时间时不会根据 Access 设置猜测待审批。任务卡片、分组、角标、ARIA、设置预览和前后任务候选来自同一个 Controller 状态包，不会因版本不一致被不同界面分别清空。若旧 Controller 已经发布了空数据，正常重载后会重新读取
