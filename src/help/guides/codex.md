...
- 通过 CodexHost 拉起的额外进程（Pi、Claude Code、OMP、Cursor 等）会出现在同一张任务列表里，重载插件后也会重新读到已有会话，不必等新建或再次跑起来。正在跑或刚创建显示「进行中」；跑完后会进「已完成未读」，不会一直停在进行中。提问/提示显示「待输入」；权限/工具审批显示「待确认」；中断或失败显示「待继续」。这些会话的未读以 Host 为准；你在 Codex 里打开过，或用快捷键跳进 Codex，这里就记为已读。官方未读小点仍然管不到这些会话
