...
- **置顶＝暂存待查：** 置顶一条**已完成且已读**的任务，它会进入「动态」页最顶部的「置顶」分组，并且不再受动态小时窗口影响——不置顶的已完成任务过窗就从动态页消失，置顶的会一直留着，方便随时回看。置顶但仍在待输入/进行中/待继续/已完成未读的任务，照旧留在各自状态组，不会被抓进置顶分组。置顶对所有来源一视同仁。置顶**不会**改变任务在其他快捷键里的参与：置顶的待输入、进行中、待继续任务照旧出现在「上一个/下一个」循环和「待输入」入口里。
