...
3. **水球** — 紧凑悬浮球外观与读数
4. **卡片** — 展开面板主题（与水球配色独立）
5. **运行** — 环境诊断、连接状态、额度自动刷新秒数、明文运行诊断日志，以及 **接入来源**（跳转前确保目标应用已打开、通过 CodexHost 打开 Codex 与 codexhost 命令位置、Claude Code 开关、事件钩子注册、Claude App 额度只读授权、Cursor Agent 冷库存开关、Orca Agents 开关）。事件钩子注册一次即可，关掉或重开插件不会卸掉；配置页会在启动时回读，不必每次再点注册。
...
- 「项目」内还有会话级来源筛选：**全部 / 只显示 Codex / 只显示 Claude**，接入 Cursor 后多一项 **只显示 Cursor**，接入 Orca 后再多一项 **只显示 Orca**；默认全部；筛选会同时过滤项目内任务并重算项目数与任务数
...
- **Claude 完成状态：** 父任务和所有子任务均确认完成后，任务离开「进行中」，按未读情况进入「已完成未读」或「已完成」；新一轮开始后再进入「进行中」。
- **动态：** 展示近期（按最新 Turn 活动，默认 24 小时内）的非隐藏、非暂停任务；可在 Codex → **任务** → **动态时间筛选（小时）** 修改范围。按「待输入 → 进行中 → 待继续 → 已完成未读 → 已完成」分组，并按最近提问、创建时间、匿名 key 稳定排序。已完成的 Claude / Cursor 行在没有开着的 Turn 时改用完成或最近活动时间，不会显示「时间缺失」。普通待继续超时后退出；唯一例外是“Plan 已完成、未执行、已确认中断且未暂停”，它会稳定留在待继续分组，但仍受完整库存保留范围约束。
...
- **置顶＝行标记：** 图钉仍标在行上。进行中、待输入、待继续、已完成未读优先显示在对应状态分组；只有已完成已读和状态未知的置顶任务停在「动态」页最顶部的「置顶」分组。该分组严格按你本地调整的置顶顺序显示；后台更新时间、完成时间和子任务活动不会让置顶项跳位，只有显式 `Alt+↑/↓` 重排才会改变。图钉未置顶时直立，已置顶时斜钉 45°，不只靠颜色；本地置顶仍用警告色，应用置顶用强调色。**动态小时窗口对任何置顶任务都不生效**——不置顶的任务过窗就从动态页消失，置顶的一直留着。置顶不另改角标：待输入/进行中/待继续/已完成未读仍按真实状态计数并参与对应快捷入口。置顶标题可折叠：展开时可见任务按当前顺序取得 `Alt+数字`；折叠时任务行隐藏，标题只显示一个“展开”编号，触发它只展开，展开后编号立即重算，不为隐藏任务留空号。`Alt+F` 仍只跳任务。置顶对所有来源一视同仁。**应用里的原生置顶也会进来**：Codex Desktop 侧栏 Pinned、CodexHost 额外进程的置顶、Claude App 的星标、Cursor 侧栏的 Pin 都会带上应用置顶标记；已完成已读或未知时停在置顶分组（本地置顶在前，应用置顶按应用顺序在后），其余相位进对应状态分组。点 `顶` 时，Codex / CodexHost 会写回对应应用；在 Codex Desktop 或 CodexHost 里置顶/取消也会实时进插件。Claude App 与 Cursor 只在插件里本地置顶，应用里自己置顶后会实时同步进插件（Cursor 取消置顶以该 workspace 库的 WAL 变化为准，不能只看主库是否改了），插件不会改它们的侧栏；对应用里已置顶的任务再点 `顶`，是在它上面叠加一层 EyPc 本地置顶（应用取消后它仍留在分组里），再点一次只取消这层本地置顶，应用内的置顶要回应用里取消。`Alt+↑/↓` 只调整本地置顶顺序。Codex 侧栏在 EyPc 置顶后要等窗口重新获得焦点才刷新。
...
- **任务拓扑与诊断：** 只有 exact、同 Provider、同 family、父存在、非自身、无环且 generation 不倒退的关系才会聚合，嵌套子任务统一归到根。禁止按标题、路径、时间、模型或界面位置猜关系；无效关系独立显示或匿名隔离。V1 只显示一张根卡、第二行 `sub+N` 与悬停里的活动/注意/异常数量，不展示子任务标题、正文或 transcript，也不提供子任务操作。匿名诊断只记录聚合计数、结果枚举、operationId 与会话期 taskRef，不记录原始父子身份或内容。
- **悬浮窗自恢复：** 打开 EyPc 工作台（或重新接入/安装后进入）会自动重建桌面悬浮窗并重新加载内容，不必先关掉再打开悬浮球。身份不一致、任务包失效或悬浮窗报告的版本比主插件旧，也会自动重建；只有重建后仍对不上才显示「需要重载」。心跳仍每 2 秒检查，连续超过 6 秒无响应时在 60 秒冷却外受控重建。全局槽位、待输入、上一个/下一个等后台快捷键不会拆掉悬浮窗。任务 Snapshot 的 applied ACK 超时只重发一次并记录诊断；健康心跳下缺 ACK 不再销毁或重建窗口。每次交互都有匿名 interaction id，10 秒无活动、失焦或生命周期结束会清理，避免拖动/展开锁长期卡死。
...
- **构建与实际加载：** 每次 `pnpm run build` 都会重写 `dist/runtime-identity.cjs` 的 `builtAt/builtAtLocal/packageVersion`，它是用于打包的最新 `dist` 凭据；Codex「运行」页同时展示当前宿主实际加载产物的时间与 `host-loaded/reload-required`。重新打包只生成新的 `dist`，状态是 `artifact-ready`，不会自动替换 uTools 已加载的 ASAR。开发模式请在 uTools 开发工具重新接入 `dist/plugin.json` 并进入工作台，悬浮窗会自动重建；若「运行」页仍报 `reload-required`，再手动结束旧插件后台进程后重新进入。离线包需安装新的 UPXS。只有实际/期望 Host、Renderer、Kernel 和 Package 身份一致才是 `host-loaded`，不能从源码时间或进程启动时间推断。
...
Orca 是第四个独立来源，**默认关闭**。开启后插件只读本机 Orca Agents 的任务状态（进行中 / 已完成 / 未读 / 工作树置顶），把每个 Agent 终端列进同一任务清单。普通没有 Agent 的终端不进清单。不做额度，不读对话正文、Prompt、工具参数或终端预览。点卡片先切到该终端，再把 Orca 窗口前置（不在最前或其他桌面时也会拉到前面，效果接近 Codex 深链）。Orca 未运行时先启动。这只是「已派发」：插件不能确认你已经看见那条对话，也不会把它标成已读。归档会关闭该终端窗格；进行中的任务不能归档。插件置顶只落 EyPc 本地；Orca 工作树置顶会读进插件，但插件不会写回 Orca。

任务行第二行来源缩写是 `OR`，悬停仍说「归属 Orca」。项目名取 Orca 仓库显示名，不展示本机绝对路径。第一行抬头优先用 Orca 标签标题；工作中终端 spinner（`⠋ Grok`）不会把新对话退化成仓库名。进行中在用户一发出下一轮、终端已进入等待回复（Grok 的 `Waiting for response` / `⠋ Grok`）时就会亮，不必等第一段工具或助手输出。已完成仍跟无工作帧的 `done`。Orca 工作树未读只是工作区汇总：同一组里只有实际未读的那条会进「已完成未读」，不会把组内其它已读对话一起标未读。点卡片或在 Orca 里已读，都不会把仍在 `working` 的会话改成已完成。

## 接入 Cursor Agent（可选）

Cursor Agent 是第三个独立来源，**默认关闭**。开启后插件只读本机 Cursor `composerHeaders` 白名单与磁盘 `status`，把本机 Agent 与 Plan 模式会话列进同一任务清单（Chat / Ask / Edit 不进）。Plan 或 Agent 在向你提问、等你批准终端命令时显示为「待输入」，点一下或用快捷键就能跳过去回答；磁盘 `status=none` 且没有会话头的空壳不进清单。归档双向跟随 App 的 `isArchived`：App 里归档的会话不进清单；在插件里对待继续/已完成会话点“归”，会在写前重验无进行中证据后翻转同一个 `isArchived` 对，App 归档列表同步可见。插件里点置顶只落 EyPc 本地；Cursor 侧栏 Pin 会读进插件置顶谓词（已完成已读或未知时停在置顶分组，其余相位进对应状态分组），但插件不会写 `cursor/pinnedComposers`。侧栏取消置顶后，插件按该 workspace 库的 WAL 变化重新读入。uTools 自带 Node 读不了这份库，插件改用本机 `sqlite3` 只跑白名单查询。进行中状态需要你在设置页确认后，才会把观察脚本加法写入用户级 `~/.cursor/hooks.json`（可随时移除，失败开放）。磁盘 `status=completed` 且没有会话自己的进行中冷证据时，即使钩子还记着开着的一轮，也不再显示进行中。钩子已经收到结束（completed/stopped）后，会话上残留的进行中标记也不能单独把卡片留在进行中；若还有活着的子任务则父卡仍显示进行中。注册一次后关掉或重开插件仍保留，配置页会回读，不必每次再点注册。脚本只读 stdin 前 32KB 白名单字段；库存文件一变就补读未读/已读，不再等几秒轮询。若这份文件里已有钩子写了非法的 `loop_limit: 0`，Cursor 会拒收整份配置、热路径不会点火；重新注册会把该值收成 `1`。不做额度。点卡片会通过 Cursor 官方 deeplink（`agent?id=会话id`）唤起 Cursor 并切到该对话（Cursor 3.17.8 实测；Cursor Cursor 未运行时由就绪层先 `open -b` 启动并等到进程与窗口出现，再发送深链）。这只是「已派发」：插件不能确认对话真的展示成功，也不会把它标成已读。
...
- **状态互斥且可恢复历史**：匹配固定语法的 App 精确日志优先，唯一 official Hook 次之，`completedTurns` 恢复无更新 active 证据的历史完成；证据歧义显示 unknown。一张卡只进「待输入 / 进行中 / 待继续 / 已完成未读 / 已完成」中的一个分组。
...
- **旧任务不会被子代理尾事件复活**：只有新 Prompt 开启父 Turn；Stop 后的 SubagentStop、工具或 SessionEnd 尾事件不能把已完成任务重新显示为进行中。App 明确完成优先同一轮 Hook 尾事件。App 已经记下中断或结束后，钩子如果只多了一条提问提交、后面没有工具活动，超过约一分钟就仍按 App 显示「待继续」，不会一直停在「进行中」；这一分钟内的新提问，或随后出现的工具/审批，仍会立刻回到进行中。
...
- **完成/焦点热未读 + 持久恢复**：Claude App 日志里匹配到的精确完成与当前聚焦会话事件会立即更新 EyPc：聚焦任务完成保持已读，非聚焦任务完成进入已完成未读，聚焦到该任务立即清除；新一轮运行可再次未读。App Local Storage 的 exact `epitaxy-unread-v1` LevelDB 快照继续用于冷启动与漏事件恢复，迟到持久值不会推翻更新的热事件。EyPc 不写 Claude App，也不持久化这层热提示。Claude 的全局聚焦事件无法判断“多窗格可见但未聚焦”是否已阅读，因此这个场景不承诺与原生小点完全同步。
...
- **虚拟项目与归属：** EyPc 不改 Codex/Claude 原生项目；相同路径优先合并，只有双方名称都唯一时才按名称合并，歧义重名分开。Claude-only 项目会进入项目区，共享项目在“全部”只出现一次。任务行第二行用来源缩写 `CC` / `CX` / `CS`，Codex Host 额外进程用 `XH`，悬停和读屏仍说「归属 Claude / Codex / Cursor / Codex Host」；项目行仍显示完整「归属 Codex/Claude/共享」，并有轻量来源背景。原生 Codex（含在 Codex 里用额外模型跑的对话）只要官方仍显示进行中，即使还没有 Turn 列表也会进「进行中」，不会被当成空会话丢掉。默认的 Claude Chats / Cursor Chats / Chats 容器名不再占第二行，真实项目名紧跟来源标记（Claude 取会话目录名，Cursor 取工作区文件夹名）。Claude completed/stopped 的任务级静默归档仅在唯一文件门禁下可用；点 `顶` 只在插件本地置顶，App 里星标仍会同步进插件。Claude 项目级归档、移除和移动仍禁用并说明。
...
- Code-only 标题/历史、App log 固定语法匹配、Hook 唯一关联、LevelDB 精确键、authority lane 隔离、blocked quota 下状态发布、Epitaxy singleflight/no-clone 和动态 N-window 合并均已有确定性测试。
- D′ 已由定向文件事务测试覆盖：completed/stopped、幂等已归档、平台/phase/身份拒绝、零 Deep Link/AX/exec、非目标与 LevelDB 不变、并发写保护、安全回滚、无 App 日志成功、精确 watcher 与一秒补漏。真实 canary 未经单独确认不会执行。
