...
首次打开时，悬浮窗可能短暂显示「正在连接」，等待主插件送达状态后会自动恢复；此时无需重载。确认版本不一致时才显示「需要重载」。

## 入口

- 主窗口 **Codex** Tab
- uTools：`eypc-codex` 及相关全局功能（显示/隐藏悬浮球、直接展开卡片、快速任务查看、待输入、已完成未读、上一个/下一个任务、归档当前任务等）
- 插件内默认：`Ctrl+Alt+Q`（macOS 常为 `Command+Option+Q`）切换悬浮球；`Ctrl+Alt+Enter` 显示并展开卡片；`Ctrl+Alt+K` 进入快速任务查看。从快捷键、展开卡片或角标打开某条对话后，列表高亮会跟到这一条。「上一个 / 下一个」跨已启用来源，只循环最高的非空类别：进行中 → 待输入／待确认 → 已完成未读＋待继续 → 已完成已读的置顶项。当前类别还有合格任务，就不会进入下一类；EyPc 已隐藏或暂停的任务始终排除。点击当前循环外的任务会更新高亮，但不会把循环切成它所在的显示分组。
...
- 「展示样式」可选水球、卡片或贴边线。贴边线为两条紧贴的 4px 周额度线（总厚 8px、长 120px），内侧一体窄底板显示两路周额度及彩色状态点数量；缺少周额度时显示空轨。左右竖放，上下横放。水球或卡片拖动松手后也会自动吸附最近边缘并切换为贴边线；设置中可重新选择水球或卡片。靠近顶角向上拖可停靠顶部；macOS 顶部贴住系统允许的上沿，通常位于菜单栏下方。
- 贴边线悬停 200ms 展开任务卡片；按住线条或卡片标题空白处即可拖动。单击线条无动作，数字角标只执行原来的任务操作，按下后移出再松手会取消。固定预览用标题处的图钉；拖动时临时收起，松手恢复固定状态。Escape 或取消拖动恢复起点。
- 贴边位置按屏幕及沿边相对位置记忆；原屏幕暂时断开时借用其它屏幕，接回后归位，主动跨屏拖动才改变归属。顶部线条贴屏幕最上沿，角标和卡片向下展开；系统栏覆盖及命中以当前 uTools 宿主表现为准。
- 紧凑水球：裁去窗口顶部 5px 留白，球体保持原比例；显示额度读数与角标计数（待输入 / 进行中相关 / 已完成未读等；为 0 的角标会隐藏，超过 99 显示 `99+`）
...
- 紧凑形态的底部整个下半区（含透明两侧和边距）均可按住拖动，悬停显示抓手、按住显示抓紧；数字角标仍是独立按钮。水球上方三分之一展开任务卡片，贴边线悬停展开。
...
- 指针/焦点离开时约 220ms 自动收起；编辑器、详情/抽屉、Quick Jump、固定预览、拖动或调整尺寸期间会暂缓
...
- **Codex / Codex++ 完成状态：** 同样看整组。原生侧栏还在转圈、分屏子窗还在 Working，或 Cloud Goal 尚未 complete 时，根卡停在「进行中」。主窗某一轮 Turn 结束不能单独把任务送进已完成组；父和全部子都结束后，才按未读进入「已完成未读」或「已完成」。子任务不上独立卡片，只在根卡显示 `sub+N`。
...
- **上一个 / 下一个任务：** 每次只在最高的非空类别中循环，依次是：①进行中；②待输入／待确认（含当前 Plan 选择或实施请求）；③已完成未读＋待继续（含未执行的 Plan）；④已完成已读的置顶项（本地或应用置顶）。当前类别全部退出后才进入下一类；有更高类别的合格任务出现，就立即切回更高类别。打开或看过一个进行中、待输入或待继续任务，不会让它退出；已完成未读在快捷打开成功派发后，按既有规则变为本轮已读并重新筛选。待继续保留原来的状态和列表分组，只在此循环中与已完成未读同级。角标仍各自计数，待输入、已完成未读专用快捷键保留原有行为。EyPc 已隐藏、暂停、已归档、不可打开、子任务及关闭来源一律不进入循环；普通已完成已读和状态未知的置顶项也不进入。活动时间窗与置顶豁免保持原规则。每层跨来源按最近提问时间倒序，同时间保持稳定顺序。连续按键约 4 秒内保持同层幸存任务的遍历顺序，新合格项加入末尾；隐藏、退出当前层或更高层出现会立即更新资格，取消尚未派发的失格目标。已派发的原生打开无法撤回，迟到结果不会夺回已失格任务的循环位置。第一下立即派发，打开过程中每次连按继续前进一格，合并成最后一个目标派发。卡片点击、快速跳转或专用入口确认打开当前候选项后，循环从它继续；打开候选外的任务只更新高亮。当前项退出时按旧次序寻找合格邻居；整层替换则从新层边界开始。所有类别都为空时提示没有可切换任务。
...
Orca 是第四个独立来源，**默认关闭**。开启后，每个已有对话的 Agent 终端进入同一任务清单；普通终端和仅开启工具栏的空窗格不进入。在 Orca 关闭的窗格也会移出清单。不读取对话正文、Prompt、工具参数或终端预览，不做额度。

点卡片会切到对应终端并前置 Orca 窗口；Orca 未运行时先启动。跳转结果表示「已派发」。仍在进行中的任务不会因为点击变成已完成；归档会关闭该窗格，进行中不能归档。

未读根据 Orca 的能力同步：有按窗格原生未读时，卡片跟随该窗格，点击跳转后等待 Orca 的已读状态传回；旧版缺少这个字段时，插件会记住自己观测到的「进行中 → 完成」，先显示已完成未读，你从插件打开后才清除这次临时未读。Claude 只要 Orca Agents 还是进行中（含后台监控、工具间隙），插件都保持进行中；只有 Agents 变成已完成才进入已完成未读。多条已完成时不按标签位置或时间猜未读，也不把工作区汇总分给每张卡。

置顶以 Orca 标签为单位，同一标签中的分屏共享钉，工作区内其他标签不受影响。支持原生置顶读写的版本会双向同步；插件写入后核对同一标签才提示同步成功。旧版可读取本机保存的标签钉；写回能力缺失或核对失败时，只保留插件本地偏好。2026-09-15 核验的 Orca 1.4.202 尚未提供原生未读导出和标签置顶命令；新代码已本地构建，完整双向效果待新版宿主加载后验收。

任务来源缩写为 `OR`，悬停为「归属 Orca」。项目名取仓库显示名（Agent 开在哪个工作树就归哪，不按标题里的 KM/EYPC 前缀改归属），不显示本机绝对路径。抬头优先使用 Orca 标签标题，工作中 spinner / `◑` 不替代主题。侧栏信封是未读铃，不能单独把仍在工作的窗格改成已完成。相位跟随原生状态及终端工作帧；Grok 进入等待回复时即可显示进行中。Claude 在 Orca 里只要还是 working，即使标成监控也仍是进行中；Agents 已经完成后，窗格残留的 `✳` 不再把卡片钉在进行中，会按未读进入「已完成未读」或「已完成」。排序和行上时间以这一轮提问时间为准，回复继续刷新不会把卡片往上抬。
...
用全局/局部快捷键、未读快捷入口、上一下一或快速跳转打开任务时，成功派发后，EyPc 立即把实际选中的已完成未读项显示为已读。适用于 Codex、Claude、Cursor、Orca，不等待下一轮刷新；跳转失败不清。进行中/等待状态保持原样；新一轮任务、子会话动态或插件重新初始化会重新按来源状态判断。这是 EyPc 本轮查看记录，不代表原生应用已经确认窗口可见或同步已读。
