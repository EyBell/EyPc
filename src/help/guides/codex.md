...
- **上一个 / 下一个任务：** 只在 V7 Snapshot 的根任务中按首个非空层循环：当前待输入/审批 → 可执行 Plan artifact（待继续）→ 动态窗口内进行中 → 未停止的本地置顶。子任务、暂停、普通隐藏和已归档全部排除；每层按最近提问时间倒序。第一下立即派发，只有首个打开仍在执行时后续连按才合并成最终尾随根目标。热 Snapshot 直接使用，冷启动/重连/缺口才执行 tasks-only 预检。
