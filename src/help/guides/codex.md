...
额度悬浮球与任务收件箱：在独立 Tab 配置外观与快捷方式，用桌面悬浮窗查看额度与近期任务状态，并通过统一任务动作内核打开或归档 Codex/Claude 会话。
...
- **动态：** 展示近期（按最新 Turn 活动，默认 24 小时内）的非隐藏任务与状态分段；可在 Codex → **任务** → **动态时间筛选（小时）** 修改范围。普通分段仍按原显示顺序；「待输入 / 已完成未读」按状态刚出现的时间倒序，最新始终在前，置顶和 Codex/Claude 来源不会压过更新状态
- **刷新周期：** Codex → **运行** → **额度刷新（秒）** 默认 300 秒；Codex → **任务** → **完整校对频率（秒）** 默认 15 秒。两者都可输入 `0–86400` 的整数，`0` 只表示对应的额度/完整库存改为手动。待输入双向热通路没有可调频率：只要 Codex 功能和任务收件箱启用，即使完整校对为 `0` 或很大、切到其它 Tab、隐藏悬浮窗，也会持续接收请求新增与解除；1 秒轻量复核只检查已登记候选，不读取完整库存或额度。
...
- **待继续：** 内部仍是 stopped，但卡片和动态分段统一显示“待继续”；不新增顶层 Tab、角标或待继续专属快捷入口。它不计入进行中角标，可任务级归档。若最新 Turn 是精确 `interrupted/user-stopped`，EyPc 先保留仍未解决的输入/审批，再检查是否已有因果上更新的新 Turn/active；两者都没有时立即进入待继续，不再等待额外 Desktop idle，也不会伪造 idle。普通 failed 仍保留原有空闲/退出门禁。Codex 写入前会再次确认 stopped 边界，若已恢复运行则拒绝。
...
- **状态时机：** 全新任务仍会先安全登记项目与匿名身份；若事件到达时完整列表暂时还没出现它，Preload 会按该脏任务精确读取一次，并且只有项目、最新 Turn 与匿名动作门禁全部通过才同时加入卡片和角标，不会先做一个假数字。首次读取未命中时，仍为 live active 的未登记状态会留在 Preload 等真实列表追上；任务偶尔从完整清单暂缺时，既有匿名活动映射会在同一数据源下短暂保留，避免隔离复查期间把仍在运行的任务误判为停止。原请求 owner 消失后，安全恢复出的有限输入决定会携带独立来源进入同一状态包；普通连接器的 active/waiting 提示仍保守为进行中。已在任务池中的任务收到完整新 Turn 时，较早的请求与 runtime waiting flag 会跨同一因果屏障立即清除，进行中卡片与角标直接同步；旧 snapshot、read-state、无关 patch、refollow 和 rollout resume 不得让它回跳，只有屏障后真正出现的新请求实例才重新进入待输入。完整且新鲜的完成 Turn 按同一 Turn 的状态版本直接切换，不会因服务端秒级完成时间与本机毫秒观测时间无法严格比较而丢失。精确完成事件即使暂时没附完整 Turn，也会先对这一条任务立即读取并短间隔复核，不再先做全量库存扫描；若期间出现更新的 active/Turn-started/等待事件，旧完成读取会失效。曾被中断的同一 Turn 恢复时即使沿用原开始时间、又漏掉中间进行中事件，最新明确 completed 仍会前进到已完成。Desktop 首次订阅若重放一个与最新 terminal Turn 冲突的 active，EyPc 会在短暂定向核验期间先保守显示进行中，只有状态 revision 未变化、没有更新的明确进行中证据且最终读取仍 terminal 才一次切换；失败/中断冲突只会降为无法确认，不会伪造空闲并误入“待继续”。任何真实新 Turn、等待请求、活动更新或读取失败都会保留/恢复进行中；即使活动本身未变化，新出现的 Plan/权限等待也会立即进入待输入并覆盖旧完成证据。反向也一样：新到达的 App Server 进行中证据不会再被旧的 Desktop idle 或 waiting 活动记录、未读变化、重新订阅或完整扫描回放；只有真正后到的新等待、停止活动或明确 terminal 证据才能改变它
...
- **多分支与诊断：** 一个 Side Chat 结束时，如果主任务或另一个 Side Chat 仍在运行，父任务继续显示进行中，不会被单个分支的晚到终态改成待继续/已完成。「运行」中的“状态裁决”常驻只显示保护合计与周期数，聚焦或悬停信息按钮可查看五项匿名明细；只有计数确实变化时刷新，普通轮询不会重复播报，也不含任务 ID、名称、正文或路径
- **版本提示：** `task-state-v9` 只表示 Codex Provider 输入兼容；任务切换的最终权威是 `companion-task-kernel-v1`，Main/Float 共用的原子快照是 `companion-task-package-v1`。任务、任务形式、分组、三个角标、前后循环、操作和跳转能力必须来自同一个 package revision。v8 及更旧输入会保留兼容数据并提示降级；Main UI、Main Preload、Float UI、Float Preload 的 Host/Renderer/Kernel/Package 身份任一不一致时会明确显示“需要重新接入或重载”并停止任务操作，不再表现为无响应。
...
- **归档：** 任务级归档支持明确已完成和待继续。界面只提交动作意图，统一 Dispatcher 再按 Codex/Claude 来源分发；归档中卡片保留并显示处理中，只有 Provider 核验 `archived` 后才即时移除，`failed/indeterminate` 都保留。Claude 仅在 macOS App `1.26832.0` 门禁下静默处理唯一登记的 `local_*.json`：写前重读 completed/stopped、文件身份与 hash，只把目标对象的 `isArchived` 改为 true，经同目录临时文件核验后原子替换；不打开 Claude、不点辅助功能、不写 LevelDB 或其它会话。元数据 true 且活动库存移除即成功，App 归档日志不是硬条件；成功只表示 EyPc 已归档并从 EyPc 列表移除，提示会同时说明 Claude 原生侧栏可能仍待刷新、当前尚未确认同步。EyPc 不强刷或伪造 Claude 原生侧栏收敛；项目级「归档已完成」仍只处理 Codex 已完成任务，不包含待继续或 Claude。
- **归档当前任务快捷调用：** `eypc-companion-archive` 保持 `mainHide`，不切换当前 Tab，也不打开 Claude。它优先使用仍可归档的当前聚焦任务，否则取注意顺序中的首个可归档任务；没有唯一候选时不写入。第一次调用只展示目标并开启 5 秒确认，第二次必须匹配同一 Provider、任务、revision、phase 与焦点才执行，任一变化都会取消。
- **上一个 / 下一个 Codex 任务：** 按独占优先级循环：先处理普通待输入/待审批；这些任务清空后，才在已完成规划、等待实施确认的 Plan 之间逐项回绕；Plan 清空后才进入动态时间筛选范围内的非隐藏进行中。普通层都为空时回退到当前未停止的 EyPc 本地置顶任务。卡片点击和静默快捷键提交完全相同的任务意图，Preload 会从最新原子包重新解析完整目标；Renderer 从未挂载也能切换，之后 Alt+Tab 不会补触发。Codex 与 Claude 同时启用时，冷状态必须等两边 tasks-only 读取都成功；600ms 后显示一次进度提示，最迟 5 秒明确失败并保留旧状态，绝不使用部分集合误跳。快速连按会逐次推进位置，但只打开约 75ms 内的最终目标，且两种桌面跳转不会并行。普通隐藏插件不会清掉该进程内位置，完全退出、停用功能或切换来源后会重新建立。两者只发起共享打开动作，不改隐藏/Tab 或 Desktop 原生状态；Host 确认打开成功后沿用上述会话期已读规则
