...
Orca 是第四个独立来源，**默认关闭**。开启后插件只读本机 Orca Agents 的任务状态（进行中 / 已完成 / 未读 / 标签置顶），把每个 Agent 终端列进同一任务清单。普通没有 Agent 的终端不进清单；只开了 Agent 工具栏、还没有对话的也不进。在 Orca 窗口里关掉的对话，浮窗里也不再列出。不做额度，不读对话正文、Prompt、工具参数或终端预览。点卡片先切到该终端，再把 Orca 窗口前置（不在最前或其他桌面时也会拉到前面，效果接近 Codex 深链）。Orca 未运行时先启动。跳转仍是「已派发」，不能确认 Orca 窗口已经看见那条对话。但若插件曾经看到它在进行中、后来已完成，卡片会先停在「已完成未读」；你从插件点开或跳转查看后，这一轮才变成已完成已读，避免直接跳成已完成而漏掉。进行中的任务点卡片不会变成已完成。归档会关闭该终端窗格；进行中的任务不能归档。Orca 标签置顶与 EyPc 置顶互通：钉的是这一条对话，不是整个工作区；在 Orca 标签栏置顶或在 EyPc 置顶/取消都会写到对方。

任务行第二行来源缩写是 `OR`，悬停仍说「归属 Orca」。项目名取 Orca 仓库显示名，不展示本机绝对路径。第一行抬头优先用 Orca 标签标题；工作中终端 spinner（`⠋ Grok`）不会把新对话退化成仓库名。进行中在用户一发出下一轮、终端已进入等待回复（Grok 的 `Waiting for response` / `⠋ Grok`）时就会亮，不必等第一段工具或助手输出。已完成仍跟无工作帧的 `done`。Orca 工作树未读只是工作区汇总，不得扇出到组内每张卡。若 CLI 给出按窗格 `unread`，卡片跟窗格，不会因为工作区汇总已是已读就把仍未读的完成对话打成已读；没有这个字段时，只有这一棵工作树里恰好一条已完成对话才记未读，不会把旁边已读的标签猜成未读。点卡片或在 Orca 里已读，都不会把仍在 `working` 的会话改成已完成。Orca 标签栏的已完成未读现网还不随 CLI 导出；没有按窗格字段时，插件用自己的中转记住「进行中 → 完成」，直到你从插件看过。清单排序和行上时间跟你发出这一轮的时间，不会因为回复过程中终端还在刷而把卡片往上抬。
