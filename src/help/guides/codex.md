...
任务来源缩写为 `OR`，悬停为「归属 Orca」。项目名取仓库显示名（Agent 开在哪个工作树就归哪，不按标题里的 KM/EYPC 前缀改归属），不显示本机绝对路径。抬头优先使用 Orca 标签标题，工作中 spinner / `◑` 不替代主题。侧栏信封是未读铃，不能单独把仍在工作的窗格改成已完成。相位跟随原生状态及终端工作帧；Grok 进入等待回复时即可显示进行中。Claude 在 Orca 里只要还是 working，即使标成监控也仍是进行中。排序和行上时间以这一轮提问时间为准，回复继续刷新不会把卡片往上抬。
