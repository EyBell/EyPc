...
未读根据 Orca 的能力同步：有按窗格原生未读时，卡片跟随该窗格，点击跳转后等待 Orca 的已读状态传回；旧版缺少这个字段时，插件会记住自己观测到的「进行中 → 完成」，先显示已完成未读，你从插件打开后才清除这次临时未读。Claude 主回复结束后若 Orca 还在监控后台任务，插件也按已完成处理，不会一直停在进行中，也不会把未读记到旁边已经结束的对话。多条已完成时不按标签位置或时间猜未读，也不把工作区汇总分给每张卡。
...
任务来源缩写为 `OR`，悬停为「归属 Orca」。项目名取仓库显示名（Agent 开在哪个工作树就归哪，不按标题里的 KM/EYPC 前缀改归属），不显示本机绝对路径。抬头优先使用 Orca 标签标题，工作中 spinner / `◑` 不替代主题。侧栏信封是未读铃，不能单独把仍在工作的窗格改成已完成。相位跟随原生状态及终端工作帧；Grok 进入等待回复时即可显示进行中。Claude 主轮次结束后若仍显示后台监控，按已完成而不是进行中。排序和行上时间以这一轮提问时间为准，回复继续刷新不会把卡片往上抬。
