...
3. **水球** — 紧凑悬浮球外观与读数
4. **卡片** — 展开面板主题（与水球配色独立）
5. **运行** — 环境诊断、连接状态、额度自动刷新秒数、明文运行诊断日志，以及 **接入来源**（Claude Code 开关、事件钩子注册、Claude App 额度只读授权、Cursor Agent 冷库存开关）
...
- **刷新机制：** Provider 的可信热事件按 membership、phase、unread、Plan、metadata、topology 六条独立 generation lane 提交 V6 evidence batch，由同一个进程级 Kernel V6 Snapshot 对外发布；普通推送不读取额度、环境或全量任务。冷启动、重连或明确成员缺口时才读取所需 Provider 的冷快照，进程退出后从来源重建。Main 隐藏、Float 关闭或 Renderer 重建都不清任务缓存。任务库存、根卡、命令、导航和 Provider 读取不设产品级总条数上限；Codex 每页 `100` 只是协议分页大小。页面不提供手动全量刷新、完整校对周期或 `Ctrl+R`；额度刷新与任务状态独立。
...
- **已完成未读：** 同样按完成状态出现时间从新到旧依次发送打开请求；Host 接受精确 Deep Link 派发后推进本地循环，失败不推进。全部当前项都派发后，下次从最新项重新开始；进度跨 EyPc 重载保留，新 Turn 完成会成为新的未派发实例。派发只表示请求已交给系统，界面会提示等待 Codex 原生确认；它不会把当前任务或 Side Chat 标成已读。未读只随 Codex 原生 read-state/持久状态变化，当前始终请求打开主任务，不直达 Side Chat。
...
- **隐藏、置顶、折叠与别名：** 这些都是 EyPc 本地偏好，不修改任何 Provider 任务；它们是任务侧唯一持久化内容。任务 alias 是 Main/Float 的统一显示名称，Provider 改标题只更新 `originalTitle`，不会顶掉 alias；清空 alias 后恢复最新原始标题。本地 alias 不同步回 Provider，V1 不提供子任务重命名。动态 membership、phase、unread、topology、Provider health 与循环位置不落盘。
- **Plan 暂停与执行：** 已完成且可继续的 Plan 使用“暂/恢”，不走普通隐藏；暂停后会进入“已隐藏”页顶部“已暂停”，并从角标和所有快捷循环移除，刷新、重启或重新关注都不会自动恢复。Plan 刚完成且原生未读时先显示“已完成未读”；读过后，只要选择/执行 Plan 的卡片仍在，就显示“待输入”。用户补充说明或新 Turn 一开始立即显示“进行中”，但 Plan lifecycle 继续保留；普通 `serverRequest/resolved` 不能清掉它。只有精确取消、开始执行、归档或移除才清 Plan：已读取消后为“已完成”，未读取消仍为“已完成未读”。Plan 行显示 `顶/暂/归/执`，暂停后为 `顶/恢/归/执`；新建会话仍在完整操作抽屉。即使 Codex 没有产生专用 “Implement Plan” 弹窗，只要已确认 Plan 完成，“暂/执”仍保持可用；`planImplementation` 只影响循环优先级，不是菜单门禁。“执”第一次只变成“确”，5 秒内第二次才会打开原任务并启动一次执行 Turn：App Server 能确认 default mode 与当前模型时使用原生模式，否则向同一任务发送固定的“执行已完成 Plan”指令。真实工作仍在运行、存在审批/其它待决请求或同任务身份/Plan revision 已变化时，由 Host 在点击后精确预检拒绝；模型或原生能力未知不能单独禁用按钮。alias 过期只续签同一匿名 key，超时不自动重发。
- **待输入热同步：** 请求出现与移除、匹配的 `serverRequest/resolved`、回答后的 matching output、用户继续和新 Turn 都走同一个双向状态通路。新用户补充、thinking/generating、active 或 Turn-started 会立即清除它之前的等待并显示进行中，不需要等到产生实际回复；若 Desktop runtime 已回到无等待标记的进行中，同一条残留请求也不会继续占着待输入。首次出现的 `进行中 + 请求` 仍是待输入。旧快照、已读变化、重新订阅或 rollout 重放不能把已解除状态重新放回。解除后真正出现的新请求仍会立即进入待输入。未匹配的 resolved 只复核这一条任务，不会误清并发审批。自动化锁定 Kernel 热发布小于 50ms；统一打开路径 P95 小于 200ms。漏一次 Activity 或 rollout 文件通知时才由 1 秒 watchdog 在 1.25 秒内补回。Float 是否真正显示以 applied ACK 为准；revision/owner/载荷缺口只重订该任务，失败时保持现状并提示降级，不按超时猜测已解除。
- **状态时机：** 新 membership 先建立最小成员，再由 V6 Kernel 在同一语义事务里更新根卡、Float、全局角标与循环队列。Topology V2 只提供 root/member 关系，不再聚合状态；Kernel 独占成员因果、根 phase、三态 unread、Plan、能力、分组和计数。成员先按各自 Turn/interaction 因果归约，再按“待审批 → 待输入 → 进行中 → Goal → 终态”聚合；任一精确成员仍活动，根任务就不能结束。任一成员未读则根未读，只有全部明确已读才为已读，其余为 unknown。旧 generation、旧 terminal、旧 running 或已越过清除屏障的 waiting 都不能覆盖更新证据。Kernel 仍无法裁决时只显示中立“核验中”，不会从任务库存恢复旧的进行中、待继续、未读或归档能力。一次语义变化只增加一次 Snapshot revision，Main、Float、角标和前后切换都消费同一 revision；不存在 Renderer 防抖或来源专用状态缓存。
...
- **版本提示：** 当前为 `task-state-v11 / companion-provider-registry-v1 / companion-task-topology-v2 / companion-task-kernel-v6 / companion-task-snapshot-v6 / companion-task-command-v1 / companion-task-subscribe-v1 / companion-task-ack-v2`。Runtime Identity 同时验证 Registry、Topology、Snapshot、Command、Subscribe 与 ACK；旧 Host、缺方法或任一身份不一致都返回 `reload-required`，不静默降级。
...
- **上一个 / 下一个任务：** 只在 V6 Snapshot 的根任务中按首个非空层循环：普通待输入/审批 → 待实施 Plan 与未执行中断的 Plan → 动态窗口内进行中 → 未停止的本地置顶。子任务、暂停、普通隐藏和已归档全部排除；每层按最近提问时间倒序。第一下立即派发，只有首个打开仍在执行时后续连按才合并成最终尾随根目标。热 Snapshot 直接使用，冷启动/重连/缺口才执行 tasks-only 预检。
...
Claude Code 是独立 Provider，可与 Codex/Cursor 各自开关并共享同一个水球。**默认只开启 Codex**，此时插件完全不读取任何 Claude 数据。
...
- **统一实时状态**：Claude 状态与未读只由 Host 适配器提交给 V6 Kernel；Renderer 不再注册 Claude 任务 watcher、提供来源专用“同步状态”动作或维护第二套计数。打开请求只走统一 Command，`dispatched` 不等于原生已打开/已读；没有匹配 native receipt 时 Provider unread 保持不变。
