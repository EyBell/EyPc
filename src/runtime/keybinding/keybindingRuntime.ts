...
export const DEFAULT_SHORTCUT_PROFILES_BY_COMMAND = {
  ...
  'windows.activate': { title: '激活当前主窗口', group: '窗口跳转', layer: 'windows', shortcutIds: ['Enter'], when: "tab == 'windows' && !windowEditorOpen && !windowActionsOpen && (!textInputFocused || activeInputRole == 'window-search')", weight: 150, profileId: 'windows' },
  'windows.tree.expand': { title: '展开或进入文件管理器子窗口', group: '窗口跳转', layer: 'windows', shortcutIds: ['ArrowRight'], when: "tab == 'windows' && !windowEditorOpen && !windowActionsOpen && (!textInputFocused || activeInputRole == 'window-search')", weight: 152, profileId: 'windows' },
  'windows.tree.collapse': { title: '收起或返回文件管理器父节点', group: '窗口跳转', layer: 'windows', shortcutIds: ['ArrowLeft'], when: "tab == 'windows' && !windowEditorOpen && !windowActionsOpen && (!textInputFocused || activeInputRole == 'window-search')", weight: 152, profileId: 'windows' },
