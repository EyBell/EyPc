...
import { toggleIdWithAdvance } from '../domain/listSelection'
...
import {
  ...
  candidateWindowRowId,
  chooseFileManagerGroupLanding,
  fileManagerGroupKey,
  fileManagerGroupRowId,
  liveWindowRowId,
  reconcileWindowTargetsWithFamilies,
  resolveVisibleWindowTreeFocus,
  resolveWindowTreeActionTargets,
  resolveWindowTreeNavigation,
  targetWindowRowId,
  toggleWindowTreeSelection,
  windowActionsContextFor,
  type WindowActionsContext,
  type WindowActionsMode,
  type WindowRow
...
export interface AppRuntimeSnapshot {
  ...
  windowActionsMode: WindowActionsMode
  windowActionsContext: WindowActionsContext
  windowActionSlot: number | null
...
export type WindowDraftField = 'alias'
...
/** Development-only session record. The title is display-only and never participates in identity matching. */
...
export function createAppRuntime(initialState: AppState, options: AppRuntimeOptions = {}) {
  ...
  let windowFreshMemberIds = new Set<string>()
  ...
  let windowActionsMode: WindowActionsMode = 'single'
  let windowActionsContext: WindowActionsContext = 'window'
  let windowActionSlot: number | null = null
  let expandedWindowGroupKeys = new Set<string>()
  const recentFileManagerWindowByGroup = new Map<string, string>()
  ...
  const windowActivationDiagnosticMessages: Record<WindowActivationDiagnosticCode, string> = {
    ...
    'target-unobserved': '当前桌面未观察到目标窗口，已保留唯一实例与快捷槽缓存。',
    'target-indeterminate': '当前无法确认目标窗口状态，已保留唯一实例与快捷槽缓存。',
    ...
    'rebind-required': '原窗口实例已失效，请在同一应用的候选窗口中明确确认；标题仅供辨认。',
    'instance-mismatch': '当前窗口实例与保存目标不一致，需要重新确认。',
    'member-mismatch': '指定子窗口已失效或不再属于该主窗口；EyPc 未回退打开主窗口或兄弟窗口。',
    'identity-unavailable': '无法建立稳定的系统窗口实例身份，未执行收藏、绑定或激活。',
    'space-unbound': '无法唯一确认目标窗口所在桌面，已保留窗口身份和快捷槽缓存。',
    'space-ambiguous': '目标窗口映射到多个远端桌面，EyPc 未执行不确定切换。',
    'space-switch-timeout': '目标桌面切换未在时限内得到确认，未前置同应用其他窗口。',
  ...
  const nativeTraceDetails = new Set<NonNullable<WindowOperationTrace['steps'][number]['detail']>>([
    'instance-match', 'instance-mismatch', 'identity-unavailable', 'focus-state-mismatch',
    'root-family-match', 'ax-cg-id-match', 'ax-focused-root-window', 'session-cache',
    'direct-space-binding', 'reverse-space-binding', 'space-switch-confirmed', 'error'
  ])
  ...
  function currentWindowBridge(capability: WindowCapability): boolean {
    return !capability.supported
      || (capability.platform !== 'darwin' && capability.platform !== 'win32')
      || capability.bridgeRevision === WINDOW_BRIDGE_REVISION
  }

  function staleWindowBridgeCapability(capability: WindowCapability): WindowCapability {
    return {
      ...capability,
      canList: false,
      canActivate: false,
      ...
      reason: windowActivationDiagnosticMessages['bridge-stale']
    }
  }

  function applyWindowInventory(freshFamilies: WindowFamily[], completeness: 'complete' | 'partial') {
    const inventory = applyWindowInventoryUpdate(windowFamilies, freshFamilies, completeness)
    windowFamilies = inventory.families
    liveWindows = inventory.roots
    windowInventoryCompleteness = inventory.completeness
    windowFreshLiveIds = inventory.freshRootIds
    windowFreshMemberIds = inventory.freshMemberIds
    windowListLoaded = true
    windowCacheUpdatedAt = Date.now()
  }

  function resetWindowInventory() {
    windowFamilies = []
    liveWindows = []
    windowFreshLiveIds = new Set()
    windowFreshMemberIds = new Set()
    windowInventoryCompleteness = null
  ...
  function fileManagerTargetByGroupKey(groupKey: string | null | undefined): WindowTarget | null {
    if (!groupKey) return null
    return state.windowTargets.find((target) => target.scope === 'file-manager-group' && target.groupKey === groupKey) || null
  }

  function fileManagerWindows(groupKey: string, freshOnly = false): LiveWindow[] {
    return liveWindows.filter((live) => fileManagerGroupKey(live.platform, live.appId) === groupKey
      && (!freshOnly || windowFreshLiveIds.has(live.instanceId)))
  }

  function windowFamilyForRoot(rootInstanceId: string, freshOnly = false): WindowFamily | null {
    const family = windowFamilies.find((candidate) => candidate.root.instanceId === rootInstanceId) || null
    if (!family || (freshOnly && !windowFreshLiveIds.has(rootInstanceId))) return null
    return family
  }

  function exactWindowChild(rootInstanceId: string, memberInstanceId: string, freshOnly = false): LiveWindow | null {
    const family = windowFamilyForRoot(rootInstanceId, freshOnly)
    return family?.children.find((child) => child.instanceId === memberInstanceId
      && (!freshOnly || windowFreshMemberIds.has(child.instanceId))) || null
  }

  function liveWindowsForTarget(target: WindowTarget, freshOnly = false): { live: LiveWindow | null; candidates: LiveWindow[] } {
    return resolveLiveWindowsForTarget(target, liveWindows, {
      freshOnly,
      freshInstanceIds: windowFreshLiveIds,
      inventoryCompleteness: windowInventoryCompleteness
    })
  }
  ...
  function windowRows(): WindowRow[] {
    return buildWindowTreeRows({
      targets: state.windowTargets,
      slots: state.windowSlots,
      liveWindows,
      windowFamilies,
      freshInstanceIds: windowFreshLiveIds,
      freshMemberInstanceIds: windowFreshMemberIds,
      currentPlatform: currentWindowPlatform(),
      listLoaded: windowListLoaded,
      focusedRowId: focusedWindowId,
      selectedRowIds: selectedWindowIds,
      expandedGroupKeys: expandedWindowGroupKeys,
      recentFileManagerInstanceIds: recentFileManagerWindowByGroup,
      searchQuery: state.windowSearch,
      rebind: windowRebindView(windowRebindState)
    })
  }
  ...
  function resetWindowActionsState() {
    windowActionsOpen = false
    windowActionTargetId = null
    windowActionsMode = 'single'
    windowActionsContext = 'window'
    windowActionSlot = null
  }

  function openWindowActionsState(input: {
    targetId: string | null
    mode: WindowActionsMode
    context: WindowActionsContext
    slot?: number | null
  }) {
    windowActionTargetId = input.targetId
    windowActionsMode = input.mode
    windowActionsContext = input.context
    windowActionSlot = input.slot ?? null
  ...
  function normalizeFocusedWindow(notifyAfter = true) {
    ...
    if (windowActionTargetId && !rows.some((row) => row.id === windowActionTargetId)) {
      resetWindowActionsState()
    }
    if (notifyAfter) notify()
  }

  function restoreWindowTreeFocus(previousRow: WindowRow | null, notifyAfter = true) {
    const rows = windowRows()
    focusedWindowId = resolveVisibleWindowTreeFocus(previousRow, rows)
  ...
  function rematchFocusedWindowAfterRefresh(previous: { rowId: string | null; live: LiveWindow | null; targetId: string | null }) {
    ...
    if (previous.rowId && rows.some((row) => row.id === previous.rowId)) {
      ...
      if (windowActionsOpen) {
        const row = rows.find((candidate) => candidate.id === previous.rowId)
        if (row) syncOpenWindowActionsToFocusedRow(row)
      }
      return
    }
    if (previous.targetId) {
      ...
      if (targetRow) {
        ...
        if (windowActionsOpen) syncOpenWindowActionsToFocusedRow(targetRow)
        return
      }
    }
    if (previous.live) {
      const liveMatch = liveWindows.find((live) => live.instanceId === previous.live!.instanceId)
      if (liveMatch) {
        const liveRow = rows.find((row) => row.live?.instanceId === liveMatch.instanceId)
        focusedWindowId = liveRow?.id || liveWindowRowId(liveMatch.instanceId)
        if (windowActionsOpen && liveRow) syncOpenWindowActionsToFocusedRow(liveRow)
        return
      }
      if (previous.live.relationship === 'child' && previous.live.rootInstanceId) {
        const parentRow = rows.find((row) => row.kind === 'window' && row.live?.instanceId === previous.live!.rootInstanceId)
        if (parentRow) {
          focusedWindowId = parentRow.id
          if (windowActionsOpen) syncOpenWindowActionsToFocusedRow(parentRow)
          return
        }
      }
    }
  ...
  function ensureWindowTarget(row: WindowRow, options: { favorite?: boolean } = {}): WindowTarget | null {
    ...
    if (row.kind === 'child-window') return null
    if (row.kind === 'file-manager-group' && row.groupKey) {
      const existingGroup = fileManagerTargetByGroupKey(row.groupKey)
      if (existingGroup) return existingGroup
      const now = Date.now()
      const groupWindows = fileManagerWindows(row.groupKey)
      const preferred = groupWindows.find((live) => live.focused)
        || groupWindows[0]
        || null
      const target: WindowTarget = {
        id: `window-group:${now}:${Math.random().toString(36).slice(2, 10)}`,
        alias: row.displayName.trim() || row.appName || '文件管理器',
        scope: 'file-manager-group',
        platform: preferred?.platform || (row.groupKey.includes(':win32:') ? 'win32' : 'darwin'),
        appId: preferred?.appId || row.appName,
        appName: preferred?.appName || row.appName,
        lastKnownTitle: preferred?.title || row.title,
        lastInstanceId: null,
        lastNativeRef: null,
        groupKey: row.groupKey,
        lastActiveInstanceId: preferred?.instanceId || null,
        alternateAliases: [],
  ...
  async function refreshWindows({ clearSearch = false }: { clearSearch?: boolean } = {}): Promise<WindowRefreshOutcome> {
    ...
    try {
      ...
      if (!currentWindowBridge(result.capability)) {
        windowCapability = staleWindowBridgeCapability(result.capability)
        message = windowActivationDiagnosticMessages['bridge-stale']
        if (!windowListLoaded) resetWindowInventory()
        normalizeFocusedWindow(false)
        return 'failed'
      }
      const freshFamilies = coalesceNativeWindowFamilies(result.windows.map((window) => ({ ...window })))
      const freshWindows = windowFamilyRoots(freshFamilies)
      const reconciliation = reconcileWindowTargetsWithFamilies(state.windowTargets, state.windowSlots, freshWindows)
      if (reconciliation.changed) {
        state.windowTargets = reconciliation.targets
        state.windowSlots = reconciliation.slots
        save()
      }
      ...
      if (windowRebindState.phase === 'confirming') {
        ...
        if (!candidateTarget) {
          ...
          const focusedCandidateInstanceId = candidateInstanceIdFromRowId(focusedWindowId)
          ...
          if (effects.probeStaleBindingTargetId) {
            const staleTarget = windowTargetById(effects.probeStaleBindingTargetId)
            if (staleTarget) {
              const proof = await probeWindowTargetInstance(staleTarget)
              if (proof.status === 'gone') {
                clearVerifiedGoneWindowNativeRef(staleTarget, proof)
                message = windowActivationDiagnosticMessages['target-closed']
              } else {
                message = windowActivationDiagnosticMessages[proof.status === 'live' || proof.liveness === 'temporarily-unobserved'
                  ? 'target-unobserved'
                  : 'target-indeterminate']
              }
            }
      ...
      if (candidateFocusInstanceId) {
        focusedWindowId = candidateWindowRowId(candidateFocusInstanceId)
  ...
  function resolveWindowEscapeStep(input: ShortcutInputContext): string | null {
    if (windowActionsOpen) {
      closeWindowActions()
      return 'windows.actions.close'
    }
    const searchFocused = input.activeInputRole === 'window-search'
    if (searchFocused && state.windowSearch) {
      const previousRow = windowRowById(focusedWindowId)
      state.windowSearch = ''
      searchBlurRequestId += 1
      restoreWindowTreeFocus(previousRow, false)
      save()
      notify()
    ...
    if (clearWindowCandidates()) return 'windows.candidates.clear'
    if (selectedWindowIds.length) {
  ...
  function diagnosticCodeForActivationOutcome(
    ...
    if (reasonCode === 'instance-mismatch') return 'instance-mismatch'
    if (reasonCode === 'member-mismatch') return 'member-mismatch'
    if (reasonCode === 'identity-unavailable') return 'identity-unavailable'
    if (reasonCode === 'space-unbound' || reasonCode === 'space-unbound-multiwindow') return 'space-unbound'
  ...
  function finishWindowActivation(
    ...
    code: WindowActivationDiagnosticCode,
    level: WindowActivationDiagnosticLevel,
    options: { focusRowId?: string | null; platformId?: WindowPlatform | 'unsupported'; messageText?: string } = {}
  ) {
    ...
    const messageText = options.messageText || diagnostic.message
    if (attempt.entry === 'slot') {
      const shown = showWindowWorkbench({ focusRowId: options.focusRowId, messageText })
      if (!shown) {
    ...
    } else if (options.messageText) setMessage(messageText)
  ...
  async function readWindowActivationCapability(attempt: WindowActivationAttempt): Promise<WindowCapability | null> {
    ...
    if (!currentWindowBridge(capability)) {
      windowCapability = staleWindowBridgeCapability(capability)
      appendWindowOperationTrace(attempt, 'capability', 'failed')
  ...
  async function probeWindowTargetInstance(target: WindowTarget): Promise<WindowInstanceProbeResult> {
    const persisted = liveWindowFromPersistedTarget(target)
    const instanceId = target.lastInstanceId || persisted?.instanceId || ''
    if (!persisted || !instanceId) {
      return { status: 'indeterminate', instanceId, liveness: 'indeterminate', reason: 'identity-unavailable' }
    }
    if (!platform.windows.probeInstance) {
      return { status: 'indeterminate', instanceId, liveness: 'indeterminate', reason: 'unsupported' }
    }
    try {
      const result = await platform.windows.probeInstance(persisted)
      return result.instanceId === instanceId
        ? result
        : { status: 'indeterminate', instanceId, liveness: 'indeterminate', reason: 'identity-unavailable' }
    } catch {
      return { status: 'indeterminate', instanceId, liveness: 'indeterminate', reason: 'native-query-failed' }
    }
  }

  function clearVerifiedGoneWindowNativeRef(target: WindowTarget, proof: WindowInstanceProbeResult) {
    if (target.scope !== 'instance') return
    if (proof.status !== 'gone' || !target.lastInstanceId || proof.instanceId !== target.lastInstanceId) return
    if (!target.lastNativeRef && !target.lastInstanceId) return
    target.lastInstanceId = null
    target.lastNativeRef = null
    target.updatedAt = Date.now()
    save()
  }

  async function resolveMissingWindowTarget(
    target: WindowTarget,
    attempt: WindowActivationAttempt,
    ...
    if (proof.status === 'gone') {
      clearVerifiedGoneWindowNativeRef(target, proof)
      return finishWindowActivation(attempt, 'resolve', 'target-closed', 'accepted', { focusRowId, platformId })
    }
    return finishWindowActivation(
      attempt,
      'resolve',
      proof.status === 'live' || proof.liveness === 'temporarily-unobserved' ? 'target-unobserved' : 'target-indeterminate',
      'blocking',
      { focusRowId, platformId }
    )
  }

  function rememberVerifiedWindowTarget(target: WindowTarget, live: LiveWindow, verifiedInstanceId: string) {
    if (target.scope !== 'instance') return false
    if (!verifiedInstanceId || verifiedInstanceId !== live.instanceId) return false
    const titleChanged = target.lastKnownTitle !== live.title
    const appChanged = target.appId !== live.appId || target.appName !== live.appName
    const instanceChanged = target.lastInstanceId !== verifiedInstanceId || target.lastNativeRef !== live.nativeRef
    if (!titleChanged && !appChanged && !instanceChanged) return false
    target.lastKnownTitle = live.title
    target.appId = live.appId
    target.appName = live.appName
    target.lastInstanceId = verifiedInstanceId
    target.lastNativeRef = live.nativeRef
    target.updatedAt = Date.now()
    return true
  }

  function rememberSuccessfulWindowActivation(live: LiveWindow, target: WindowTarget | null, verifiedInstanceId: string) {
    let changed = target ? rememberVerifiedWindowTarget(target, live, verifiedInstanceId) : false
    const groupKey = fileManagerGroupKey(live.platform, live.appId)
    if (groupKey) {
      recentFileManagerWindowByGroup.set(groupKey, live.instanceId)
  ...
  async function activateLiveWindow(root: LiveWindow, target: WindowTarget | null, attempt: WindowActivationAttempt, member: LiveWindow | null = null): Promise<WindowActivationOutcome> {
    if (attempt.trace && !attempt.trace.targetTitle) attempt.trace.targetTitle = member?.title || target?.lastKnownTitle || root.title
    ...
    try {
      result = attempt.operation === 'always-on-top'
        ? (await platform.windows.alwaysOnTop?.(root, { debugTrace: windowOperationTraceEnabled }) ?? { outcome: 'unsupported' as const })
        : await platform.windows.activate(createWindowActivationRequest(root, member), { debugTrace: windowOperationTraceEnabled })
    } catch {
      attempt.nativeReasonCode = undefined
    ...
    if (result.outcome === 'activated') {
      const legacyInstance = root.instanceId.includes(':legacy:')
      const memberMismatch = Boolean(member && result.memberInstanceId !== member.instanceId)
      if (!result.instanceId || (!legacyInstance && result.instanceId !== root.instanceId) || memberMismatch) {
        attempt.nativeReasonCode = memberMismatch ? 'member-mismatch' : 'identity-unavailable'
        appendWindowOperationTrace(attempt, 'native', 'unavailable')
        setMessage(windowActivationDiagnosticMessages[memberMismatch ? 'member-mismatch' : 'identity-unavailable'])
        return 'failed'
      }
      const verifiedLive = legacyInstance ? { ...root, id: result.instanceId, instanceId: result.instanceId } : root
      windowFreshLiveIds.add(result.instanceId)
      if (target) {
        rememberSuccessfulWindowActivation(verifiedLive, target, result.instanceId)
        advanceWindowRebind({ type: 'confirmed', targetId: target.id })
      } else rememberSuccessfulWindowActivation(verifiedLive, null, result.instanceId)
      setMessage(attempt.operation === 'always-on-top'
        ? '页面已置顶并前置'
        : member
          ? '指定子窗口已精确展开并前置'
          : '主窗口当前子窗口已展开并前置')
      ...
      if (!hidden) {
        appendWindowOperationTrace(attempt, 'visibility', 'failed')
        reportWindowActivationDiagnostic(attempt, 'visibility', 'silent-hide-failed', 'blocking', root.platform)
        completeWindowOperationTrace(attempt, 'blocking', 'silent-hide-failed', root.platform)
      } else {
        appendWindowOperationTrace(attempt, 'visibility', 'ok')
        completeWindowOperationTrace(attempt, 'success', attempt.operation === 'always-on-top' ? 'topmost-enabled' : 'activated', root.platform)
      }
      return 'activated'
    }
  ...
  function preferredFileManagerWindow(groupKey: string, target: WindowTarget | null, freshOnly = false): LiveWindow | null {
    const available = fileManagerWindows(groupKey, freshOnly)
    const orderedInstanceIds = windowRows().find((row) => row.kind === 'file-manager-group' && row.groupKey === groupKey)?.groupLiveInstanceIds
    return chooseFileManagerGroupLanding(available, {
      lastActiveInstanceId: target?.lastActiveInstanceId,
      recentInstanceId: recentFileManagerWindowByGroup.get(groupKey),
      orderedInstanceIds
    })
  }

  function rememberFileManagerLanding(target: WindowTarget | null, live: LiveWindow): boolean {
    if (!target) return false
    if (
      target.appId === live.appId
      && target.appName === live.appName
      && target.lastKnownTitle === live.title
      && target.lastActiveInstanceId === live.instanceId
    ) return false
    state.windowTargets = state.windowTargets.map((item) => item.id === target.id
      ? {
          ...
          appName: live.appName,
          lastKnownTitle: live.title,
          lastActiveInstanceId: live.instanceId,
          updatedAt: Date.now()
      }
      : item)
    return true
  ...
  async function activateFileManagerGroup(groupKey: string | null, target: WindowTarget | null, attempt: WindowActivationAttempt): Promise<boolean> {
    if (!groupKey) return finishWindowActivation(attempt, 'entry', 'activation-failed', 'blocking')
    if (attempt.trace && !attempt.trace.targetTitle) attempt.trace.targetTitle = target?.alias || target?.appName || '文件管理器'
    let live = preferredFileManagerWindow(groupKey, target)
    if (live) {
      appendWindowOperationTrace(attempt, 'cache', 'ok')
      const outcome = await activateLiveWindow(live, null, attempt)
      if (outcome === 'activated') {
        return true
      }
      if (outcome !== 'not-found') return finishActivationOutcome(attempt, outcome, fileManagerGroupRowId(groupKey), live.platform)
    } else appendWindowOperationTrace(attempt, 'cache', 'skipped')

    const platformId = target?.platform || (groupKey.includes(':win32:') ? 'win32' : 'darwin')
    const refreshed = await refreshForWindowActivation(attempt, fileManagerGroupRowId(groupKey), platformId)
    if (!refreshed) return false
    live = preferredFileManagerWindow(groupKey, target, true)
    if (!live) {
      appendWindowOperationTrace(attempt, 'resolve', 'not-found')
      const result = finishWindowActivation(attempt, 'resolve', refreshed === 'partial' ? 'refresh-incomplete' : 'activation-failed', 'blocking', {
        focusRowId: fileManagerGroupRowId(groupKey),
        platformId
      })
      setMessage(refreshed === 'partial'
        ? '文件管理器清单不完整，已保留当前父节点状态'
        : '文件管理器当前没有可用窗口；EyPc 不会自动启动 Finder/Explorer')
      return result
    }
    appendWindowOperationTrace(attempt, 'resolve', 'ok')
    const retried = await activateLiveWindow(live, null, attempt)
    if (retried === 'activated') {
      return true
    }
    return finishActivationOutcome(attempt, retried, fileManagerGroupRowId(groupKey), live.platform)
  }
  ...
  async function activateWindowTargetWithRecovery(target: WindowTarget, attempt: WindowActivationAttempt): Promise<boolean> {
    if (target.scope === 'file-manager-group') {
      return activateFileManagerGroup(target.groupKey, target, attempt)
    }
    if (attempt.trace && !attempt.trace.targetTitle) attempt.trace.targetTitle = target.lastKnownTitle
    const focusRowId = targetWindowRowId(target.id)
    const platformId = target.platform
    const cachedLive = liveWindowsForTarget(target).live || liveWindowFromPersistedTarget(target)
    if (cachedLive) {
      appendWindowOperationTrace(attempt, 'cache', 'ok')
      const outcome = await activateLiveWindow(cachedLive, target, attempt)
      if (outcome === 'activated') return true
      if (outcome !== 'not-found' || isWindowSpaceFailureReason(attempt.nativeReasonCode)) return finishActivationOutcome(attempt, outcome, focusRowId, platformId)
    } else appendWindowOperationTrace(attempt, 'cache', 'skipped')
    ...
    if (retried.kind === 'no-match') {
      return resolveMissingWindowTarget(target, attempt, focusRowId, platformId)
    }
  ...
  async function resolveAndActivateWindowTargetForAttempt(target: WindowTarget, attempt: WindowActivationAttempt, freshOnly = false): Promise<WindowResolveOutcome> {
    ...
    if (resolved.candidates.length) {
      ...
      const effects = advanceWindowRebind({
        ...
        restoreFocusRowId: targetWindowRowId(target.id),
        slotNumber: attempt.entry === 'slot' ? attempt.slot : null
      })
      if (effects.focusCandidateInstanceId) focusedWindowId = candidateWindowRowId(effects.focusCandidateInstanceId)
      selectedWindowIds = []
      resetWindowActionsState()
      windowFocusRequestId += 1
  ...
  function liveWindowAfterRefresh(live: LiveWindow): { live: LiveWindow | null; ambiguous: boolean } {
    const exact = liveWindows.find((candidate) => windowFreshLiveIds.has(candidate.instanceId) && candidate.instanceId === live.instanceId) || null
    return { live: exact, ambiguous: false }
  }
  ...
  async function activateLiveWindowWithRecovery(live: LiveWindow, target: WindowTarget | null, attempt: WindowActivationAttempt): Promise<boolean> {
    const focusRowId = target ? targetWindowRowId(target.id) : liveWindowRowId(live.instanceId)
    const outcome = await activateLiveWindow(live, target, attempt)
    if (outcome === 'activated') return true
    if (outcome !== 'not-found' || isWindowSpaceFailureReason(attempt.nativeReasonCode)) return finishActivationOutcome(attempt, outcome, focusRowId, live.platform)
    ...
    if (!refreshed.live) {
      appendWindowOperationTrace(attempt, 'resolve', 'not-found')
      if (target) return resolveMissingWindowTarget(target, attempt, focusRowId, live.platform)
      ...
      return finishWindowActivation(attempt, 'resolve', 'activation-not-found', 'blocking', { focusRowId, platformId: live.platform })
    }
  ...
  async function activateConfirmedWindowCandidate(live: LiveWindow, target: WindowTarget, attempt: WindowActivationAttempt): Promise<boolean> {
    ...
    const slotNumber = windowRebindState.phase === 'confirming' && windowRebindState.targetId === target.id
      ? windowRebindState.slotNumber
      : null
    if (slotNumber != null) {
      // A slot recovery is intentionally non-destructive: activate the exact
      // candidate first, then repoint only the originating slot. The old
      // logical target and every other slot keep their own user-owned state.
      const activated = await activateLiveWindowWithRecovery(live, null, attempt)
      if (!activated) return false
      let replacement = state.windowTargets.find((candidate) => targetMatchesLiveWindow(candidate, live)) || null
      let changed = false
      if (!replacement) {
        replacement = createWindowTarget(live, live.title, false)
        state.windowTargets = [...state.windowTargets, replacement]
        changed = true
      } else {
        changed = rememberVerifiedWindowTarget(replacement, live, live.instanceId) || changed
      }
      const currentSlot = state.windowSlots.find((slot) => slot.slot === slotNumber)
      if (currentSlot?.targetIdByPlatform[live.platform] === target.id) {
        state.windowSlots = state.windowSlots.map((slot) => slot.slot === slotNumber
          ? { ...slot, targetIdByPlatform: { ...slot.targetIdByPlatform, [live.platform]: replacement!.id } }
          : slot)
        changed = true
      }
      advanceWindowRebind({ type: 'confirmed', targetId: target.id })
      focusedWindowId = targetWindowRowId(replacement.id)
      if (changed) save()
      return true
    }
  ...
  async function activateExactWindowChild(row: WindowRow, attempt: WindowActivationAttempt): Promise<boolean> {
    const cachedRoot = row.rootLive
    const cachedMember = row.live
    if (!cachedRoot || !cachedMember || row.kind !== 'child-window') {
      return finishWindowActivation(attempt, 'entry', 'activation-failed', 'blocking')
    }
    if (attempt.trace) attempt.trace.targetTitle = cachedMember.title
    // The native bridge switches to the root Space and revalidates the exact
    // relationship atomically. A current-Space AX refresh before this call
    // would incorrectly discard a valid off-Space member.
    const cachedOutcome = await activateLiveWindow(cachedRoot, null, attempt, cachedMember)
    if (cachedOutcome === 'activated') return true
    if (cachedOutcome !== 'not-found' || isWindowSpaceFailureReason(attempt.nativeReasonCode)) {
      return finishActivationOutcome(attempt, cachedOutcome, row.parentRowId, cachedRoot.platform)
    }
    // A native exact miss gets one bounded refresh after the Space-aware call.
    const completeness = await refreshForWindowActivation(attempt, row.id, cachedRoot.platform)
    if (!completeness) return false
    const freshRoot = windowFamilyForRoot(cachedRoot.instanceId, true)?.root || null
    const freshMember = exactWindowChild(cachedRoot.instanceId, cachedMember.instanceId, true)
    if (!freshRoot || !freshMember) {
      appendWindowOperationTrace(attempt, 'resolve', 'not-found')
      const messageText = completeness === 'partial'
        ? '子窗口关系暂时无法完整核验；EyPc 未回退打开主窗口。'
        : '指定子窗口已失效；EyPc 未回退打开主窗口或其他子窗口。'
      return finishWindowActivation(attempt, 'resolve', completeness === 'partial' ? 'refresh-incomplete' : 'activation-not-found', 'blocking', {
        focusRowId: row.parentRowId,
        platformId: cachedRoot.platform,
        messageText
      })
    }
    appendWindowOperationTrace(attempt, 'resolve', 'ok')
    const outcome = await activateLiveWindow(freshRoot, null, attempt, freshMember)
    if (outcome === 'activated') return true
    return finishActivationOutcome(attempt, outcome, row.parentRowId, freshRoot.platform)
  }
  ...
  async function activateWindowRow(rowId?: string) {
    ...
    if (row.kind === 'file-manager-group') {
      if (row.candidate || !row.groupKey) {
        return finishWindowActivation(attempt, 'entry', 'rebind-required', 'blocking')
      }
      return activateFileManagerGroup(row.groupKey, row.target, attempt)
    }
    if (row.kind === 'child-window') return activateExactWindowChild(row, attempt)
    if (row.live) {
  ...
  async function setWindowAlwaysOnTop(rowId?: string) {
    ...
    if (row.kind === 'file-manager-group') {
      setMessage('文件管理器父节点不提供窗口置顶；请展开并选择具体窗口')
      return finishWindowActivation(attempt, 'entry', 'topmost-unsupported', 'blocking')
    }
    if (row.kind === 'child-window') {
      setMessage('子窗口不提供页面置顶；请在对应主窗口上执行')
      return finishWindowActivation(attempt, 'entry', 'topmost-unsupported', 'blocking')
    }
    if (row.live) {
      if (row.target) return activateWindowTargetWithRecovery(row.target, attempt)
  ...
  function liveWindowFromPersistedTarget(target: WindowTarget): LiveWindow | null {
    ...
    const macRef = target.platform === 'darwin' ? /^(\d{1,12}):0:(\d{1,12})$/.exec(target.lastNativeRef) : null
    const windowsInstance = target.platform === 'win32' ? /^win32:(\d{1,12}):(\d{1,20})$/.exec(target.lastInstanceId || '') : null
    const instanceId = target.lastInstanceId || (macRef ? `darwin:${macRef[1]}:${macRef[2]}` : `${target.platform}:legacy:${target.lastNativeRef}`)
    return {
      id: instanceId,
      instanceId,
      ...
      pid: macRef ? Number(macRef[1]) : windowsInstance ? Number(windowsInstance[1]) : 0,
      title: target.lastKnownTitle,
      minimized: false,
      focused: false
    }
  }
  ...
  function resolveWindowActionTargets(rowId?: string) {
    return resolveWindowTreeActionTargets(windowRows(), {
  ...
  function openWindowActions(rowId?: string) {
    if (rejectUnavailableWindowInteraction('browse')) return false
    const { mode, context, targets } = resolveWindowActionTargets(rowId)
    if (!targets.length) {
      setMessage('没有可操作的窗口')
      return false
    }
    focusedWindowId = targets[0].id
    openWindowActionsState({ targetId: targets[0].id, mode, context })
  ...
  function syncOpenWindowActionsToFocusedRow(row: WindowRow) {
    if (!windowActionsOpen || windowActionsContext === 'slot') return
    windowActionTargetId = row.id
    windowActionsContext = windowActionsContextFor(row.kind, windowActionsMode)
  }

  function setWindowGroupExpanded(rowId: string | undefined, expanded?: boolean) {
    if (rejectUnavailableWindowInteraction('browse')) return false
    const row = windowRowById(rowId || focusedWindowId)
    if (!row || !row.expandable || !row.expansionKey) return false
    const nextExpanded = expanded ?? !row.expanded
    if (nextExpanded) expandedWindowGroupKeys.add(row.expansionKey)
    else expandedWindowGroupKeys.delete(row.expansionKey)
    focusedWindowId = row.id
    syncOpenWindowActionsToFocusedRow(row)
    windowFocusRequestId += 1
    notify()
    return true
  }

  function navigateFocusedWindowTree(direction: 'expand' | 'collapse') {
    if (rejectUnavailableWindowInteraction('browse')) return false
    const rows = windowRows()
    const result = resolveWindowTreeNavigation(rows, focusedWindowId, direction)
    if (!result.handled) return false
    if (result.groupKey && result.expanded != null) {
      if (result.expanded) expandedWindowGroupKeys.add(result.groupKey)
      else expandedWindowGroupKeys.delete(result.groupKey)
    }
    focusedWindowId = result.focusedId
    windowFocusRequestId += 1
    notify()
    return true
  }

  function openWindowSlotActions(slotNumber: number) {
    if (rejectUnavailableWindowInteraction('browse')) return false
    if (slotNumber < 1 || slotNumber > 10) return false
    const platformId = currentWindowPlatform()
    const slot = state.windowSlots.find((item) => item.slot === slotNumber)
    const targetId = platformId ? slot?.targetIdByPlatform[platformId] : Object.values(slot?.targetIdByPlatform || {}).find(Boolean)
    const target = windowTargetById(targetId)
    let rowId: string | null = null
    if (target?.scope === 'file-manager-group' && target.groupKey) {
      rowId = fileManagerGroupRowId(target.groupKey)
    } else if (target) {
      const live = liveWindowsForTarget(target).live
      const groupKey = live ? fileManagerGroupKey(live.platform, live.appId) : null
      if (groupKey) expandedWindowGroupKeys.add(groupKey)
      rowId = targetWindowRowId(target.id)
    }
    if (rowId && windowRowById(rowId)) focusedWindowId = rowId
    openWindowActionsState({ targetId: rowId, mode: 'single', context: 'slot', slot: slotNumber })
    windowActionsFocusRequestId += 1
    notify()
    return true
  }
  ...
  async function closeWindowRows(rowId?: string, force = false) {
    if (rejectUnavailableWindowInteraction('browse')) return false
    const rows = windowRowsForClose(rowId).filter((row) => row.kind === 'window' && row.live)
    const explicitRow = windowRowById(rowId || focusedWindowId)
    if (explicitRow?.kind === 'child-window' && explicitRow.live && explicitRow.rootLive) {
      if (force) {
        setMessage('子窗口不提供强制终止；请在主窗口上确认进程级操作')
        return false
      }
      if (explicitRow.live.canClose !== true) {
        setMessage('该子窗口未声明可安全关闭能力')
        return false
      }
      const refreshed = await refreshWindows()
      const member = exactWindowChild(explicitRow.rootLive.instanceId, explicitRow.live.instanceId, true)
      if (!member || (refreshed !== 'loaded-complete' && refreshed !== 'loaded-partial')) {
        setMessage('指定子窗口已失效；未关闭主窗口或其他子窗口')
        return false
      }
      const result = await platform.windows.close?.(member)
      if (result?.outcome !== 'closed') {
        setMessage(result?.message || '未能精确关闭指定子窗口')
        return false
      }
  ...
  function toggleWindowFavorite(rowId?: string) {
    ...
    if (row.kind === 'child-window') {
      setMessage('子窗口是会话级精确落点；收藏请作用于对应主窗口')
      return false
    }
    if (row.target) {
      ...
      if (!target.favorite) {
        ...
        if (!windowTargetById(target.id) && row.live && row.kind === 'window') {
          remapWindowRowIdentity(row.id, liveWindowRowId(row.live.instanceId))
        }
      }
      save()
      normalizeFocusedWindow(false)
    ...
    const targetRowId = row.kind === 'file-manager-group' && row.groupKey
      ? fileManagerGroupRowId(row.groupKey)
      : targetWindowRowId(target.id)
  ...
  function toggleWindowPins(rowId?: string) {
    if (rejectUnavailableWindowInteraction('browse')) return false
    const targets = resolveWindowActionTargets(rowId).targets
    if (targets.some((row) => row.kind === 'child-window')) {
      setMessage('子窗口不参与列表置顶；请操作对应主窗口')
      return false
    }
    if (!targets.length) {
    ...
    for (const row of targets) {
      ...
      state.windowTargets = state.windowTargets.map((item) => item.id === target.id
        ...
        : item)
      if (pin && !row.target && row.kind === 'window') remapWindowRowIdentity(row.id, targetWindowRowId(target.id))
      if (!pin && row.live) liveFocusByTargetId.set(target.id, liveWindowRowId(row.live.instanceId))
      changed += 1
    }
    if (!changed) {
    ...
    if (!pin) {
      ...
      for (const [targetId, liveRowId] of liveFocusByTargetId) {
        if (!windowTargetById(targetId)) remapWindowRowIdentity(targetWindowRowId(targetId), liveRowId)
      }
    }
    save()
    normalizeFocusedWindow(false)
  ...
  function beginWindowDraft(mode: WindowDraftMode, rowId?: string) {
    ...
    if (row.kind === 'child-window') {
      setMessage('子窗口不持久化别名或目标；请编辑对应主窗口')
      return false
    }
    if (row.kind === 'file-manager-group') mode = 'rename'
    const source = row.live
    const target = row.target || (row.kind === 'file-manager-group' ? ensureWindowTarget(row, { favorite: false }) : null)
    if (!source && !target) return false
    windowDraft = {
      mode,
      targetId: target?.id || null,
      sourceWindowId: source?.instanceId || null,
      sourceGroupKey: row.groupKey,
      alias: target?.alias || source?.title || '',
      appName: target?.appName || source?.appName || '',
      appId: target?.appId || source?.appId || '',
      lastKnownTitle: source?.title || target?.lastKnownTitle || '',
      activeField: 'alias'
    }
    resetWindowActionsState()
    windowFocusRequestId += 1
  ...
  function updateWindowDraft(input: Partial<Pick<WindowDraft, 'alias' | 'activeField'>>) {
  ...
  function saveWindowDraft() {
    ...
    if (!alias || !windowDraft.appId.trim()) {
      setMessage('别名和应用不能为空')
      return false
    }
    const existing = windowTargetById(windowDraft.targetId)
    const source = liveWindows.find((live) => live.instanceId === windowDraft?.sourceWindowId) || null
    const now = Date.now()
    const target: WindowTarget | null = existing
      ? {
          ...existing,
          alias,
          lastKnownTitle: existing.scope === 'file-manager-group' ? existing.lastKnownTitle : (source?.title || existing.lastKnownTitle),
          appId: existing.scope === 'file-manager-group' ? existing.appId : windowDraft.appId.trim(),
          appName: existing.scope === 'file-manager-group' ? existing.appName : (windowDraft.appName.trim() || windowDraft.appId.trim()),
          lastInstanceId: existing.scope === 'file-manager-group' ? null : (source?.instanceId || existing.lastInstanceId),
          lastNativeRef: existing.scope === 'file-manager-group' ? null : (source?.nativeRef || existing.lastNativeRef),
          alternateAliases: [],
          updatedAt: now
        }
      : source
        ? { ...createWindowTarget(source, alias), appId: windowDraft.appId.trim(), appName: windowDraft.appName.trim() || windowDraft.appId.trim(), updatedAt: now }
        : null
    if (!target) {
      setMessage('无法为缺少稳定实例身份的窗口创建目标')
      return false
    }
    state.windowTargets = existing
      ? state.windowTargets.map((item) => item.id === target.id ? target : item)
      : [...state.windowTargets, target]
    focusedWindowId = target.scope === 'file-manager-group' && target.groupKey
      ? fileManagerGroupRowId(target.groupKey)
      : targetWindowRowId(target.id)
    windowDraft = null
    windowFocusRequestId += 1
    save()
    setMessage('已保存 EyPc 窗口别名')
    return true
  }
  ...
  function focusWindowSlot(slotNumber: number) {
    ...
    const target = windowTargetById(targetId)
    if (target?.scope === 'file-manager-group' && target.groupKey) {
      focusedWindowId = fileManagerGroupRowId(target.groupKey)
    } else {
      const live = target ? liveWindowsForTarget(target).live : null
      const groupKey = live ? fileManagerGroupKey(live.platform, live.appId) : null
      if (groupKey) expandedWindowGroupKeys.add(groupKey)
      focusedWindowId = targetWindowRowId(targetId)
    }
    windowFocusRequestId += 1
    notify()
    return true
  ...
  function moveInList(direction: 1 | -1, page = false) {
    ...
    if (state.activeTab === 'windows') {
      ...
      syncOpenWindowActionsToFocusedRow(rows[next])
      windowFocusRequestId += 1
      notify()
      return
    }
  ...
  function toggleFocusedSelection(advance = true) {
    ...
    if (state.activeTab === 'windows' && focusedWindowId) {
      ...
      const rows = windowRows()
      const next = toggleWindowTreeSelection(rows, {
        focusedId: focusedWindowId,
        selectedIds: selectedWindowIds,
        advance
      })
      ...
      if (next.virtualParentBlocked) {
        const focused = rows.find((row) => row.id === focusedWindowId)
        setMessage(focused?.kind === 'child-window'
          ? '子窗口不参与批量选择；批量操作请作用于主窗口'
          : '文件管理器父节点不参与批量选择；请展开后选择具体主窗口')
      }
      const focused = rows.find((row) => row.id === focusedWindowId)
      if (focused) syncOpenWindowActionsToFocusedRow(focused)
      windowFocusRequestId += 1
      notify()
      return
    }
  ...
  function registerActions() {
    ...
    actions.register({ id: 'codex.completed-unread.hotkey.configure', title: '配置 Codex 已完成未读快捷键', group: 'Codex', risk: 'normal', scope: 'global', priority: 89, when: () => true, run: () => {
      const opened = platform.app.configureHotkey?.('打开第一个 Codex 已完成未读任务') === true
      if (!opened) setMessage('请在 uTools 设置 → 全局功能中，为“打开第一个 Codex 已完成未读任务”绑定快捷键')
      return opened
    } })
  ...
  return {
    ...
    handleShortcut(shortcutId: string, inputContext: boolean | ShortcutInputContext): string | null {
      ...
      if (state.activeTab === 'windows' && shortcutId === 'Escape') {
        const resolved = resolveWindowEscapeStep(input)
