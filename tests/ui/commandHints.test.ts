...
describe('command help hints', () => {
  it('renders shortcut hints from the top-right help trigger instead of a fixed footer', () => {
    ...
    expect(component).toContain('commandHints?.({')
    expect(component).toContain('SETTINGS_COMMAND_HINTS')
    expect(component).not.toContain("snapshot.state.activeTab === 'ports'")
    expect(component).not.toContain("snapshot.state.activeTab === 'favorites'")
    expect(readFileSync(resolve(process.cwd(), 'src/runtime/feature/ports/commandHints.ts'), 'utf8')).toContain('端口默认')
    expect(readFileSync(resolve(process.cwd(), 'src/runtime/feature/mqtt/commandHints.ts'), 'utf8')).toContain('MQTT 默认')
    expect(readFileSync(resolve(process.cwd(), 'src/runtime/feature/favorites/commandHints.ts'), 'utf8')).toContain('快速收藏')
