...
describe('favorites initialization UI', () => {
  it('offers explicit reviewed pick paths without exposing add commands in quick mode', () => {
    ...
    expect(page).toContain('favorites.slot.manager.open')
    expect(page).toContain('favorites.runner.applySuggestion')
    expect(page).toContain('favorite-runner-preview')
    ...
    expect(quickPage).toContain('quick-favorite-row-actions')
    ...
    expect(quickPage).toContain('quick-favorite-number')
    expect(quickPage).toContain('quick-favorite-number-placeholder')
    expect(app).toContain("route.tab === 'favorites' ? 'favorites.search.focus'")
    expect(app).not.toContain("document.querySelector<HTMLElement>('[data-role=\"favorite-items\"]')?.focus()")
  })
})
