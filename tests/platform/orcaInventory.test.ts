...
describe('Orca agent inventory', () => {
  ...
  it('uses CodexHost harness abbreviations in titles', () => {
    ...
    expect(inventory.displayTitle('260917-SRM-欠款审计提交推送', 'devin', 'km-srm-ref'))
      .toBe('dv · 260917-SRM-欠款审计提交推送')
    expect(inventory.displayTitle('devin', 'devin', 'km-srm-ref')).toBe('dv · km-srm-ref')
