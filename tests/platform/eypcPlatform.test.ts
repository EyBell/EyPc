...
describe('browser fallback platform', () => {
  ...
  it('maps macOS AX elements to the exact CG id before the title compatibility fallback', () => {
    ...
    expect(activation).toContain("addTrace('space', 'ok', 'single-window-frontmost')")
    expect(activation).toContain("addTrace('verify', 'unavailable', 'focus-state-mismatch')")
    ...
    expect(activation).toContain("cgIdentity.outcome === 'title-mismatch'")
    expect(activation.indexOf("cgIdentity.outcome === 'title-mismatch'")).toBeLessThan(activation.indexOf('const exact = resolveExactAxTarget()'))
    expect(exactActivation).not.toContain("copyAxAttribute(target, 'AXTitle')")
    expect(preload.slice(preload.indexOf('const MACOS_ENV_SNAPSHOT_SCRIPT'), preload.indexOf('function macosIsolatedSpaceBridgeScript'))).toContain("replace(/\\s+/g, ' ')")
    ...
    expect(preload).not.toContain('macosActivateWindowScript(pid, ordinal, source.title)')
    ...
    expect(preload).toContain("'cg-ordinal-fallback'")
    expect(preload).toContain('function resolveCgOrdinal')
    expect(preload).toContain('resolvedByCgOrdinal')
    expect(publicPreload).toBe(preload)
  })
