...
describe('keybinding runtime', () => {
  ...
  it('maps MQTT workbench shortcuts and edit-layer ownership', () => {
    ...
    expect(resolveKeybinding(DEFAULT_KEYBINDINGS, 'Ctrl+Shift+L', idleContext)?.actionId).toBe('mqtt.log.drawer.open')
    expect(resolveKeybinding(DEFAULT_KEYBINDINGS, 'Ctrl+Shift+Enter', { ...idleContext, mqttTargetKind: 'message' as const })?.actionId).toBe('mqtt.record.repeatSend')
    expect(resolveKeybinding(DEFAULT_KEYBINDINGS, 'Ctrl+Shift+Enter', { ...idleContext, mqttTargetKind: 'publish-template' as const })?.actionId).toBe('mqtt.publish.template.send')
    ...
    expect(resolveKeybinding(DEFAULT_KEYBINDINGS, 'Ctrl+Shift+O', publishEditorContext)?.actionId).toBe('mqtt.publish.options.open')
    ...
    expect(rows.find((row) => row.commandId === 'mqtt.publish.options.open')?.defaultShortcutIds).toEqual(['Ctrl+Shift+O'])
    expect(rows.find((row) => row.commandId === 'mqtt.log.drawer.open')?.defaultShortcutIds).toEqual(['Ctrl+Shift+L'])
    expect(rows.find((row) => row.commandId === 'mqtt.record.repeatSend')?.defaultShortcutIds).toEqual(['Ctrl+Shift+Enter'])
    expect(rows.find((row) => row.commandId === 'mqtt.publish.template.send')?.defaultShortcutIds).toEqual(['Ctrl+Shift+Enter'])
